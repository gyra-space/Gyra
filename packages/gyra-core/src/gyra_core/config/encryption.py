...
class SecretsEncryption:
    ...
    主密钥来源优先级：
    1. 环境变量 GYRA_MASTER_KEY
    2. ~/.gyra/master.key 文件
    3. 自动生成并保存到 ~/.gyra/master.key
    """
    ...
    def _get_master_key_path(self) -> Path:
        return get_gyra_home() / "master.key"
    ...
    def get_master_key(self) -> str:
        ...
        env_key = os.environ.get("GYRA_MASTER_KEY")
        if env_key:
...
def get_secrets_file_path() -> Path:
    return get_gyra_home() / SECRETS_FILE_NAME
