...
from gyra.util.logger import setup_logging, LoggingParameters
...
class PeriodicAsyncProfiler:
    ...
    def _sample_main_thread_stack(self):
        ...
        filtered_stack = [
            ...
            if 'gyra' in f.filename and
               'gyra-app' not in f.filename and
    ...
    def _get_stack_frame_all(self, stack):
        ...
        for frame in reversed(stack):
            ...
            if (('multiprocess.py' in frame.filename) or
                ('gyra' in frame.filename and 'site-packages' not in frame.filename and 'periodic_async_profiler' not in frame.filename)
            ):
                break
        return frames
