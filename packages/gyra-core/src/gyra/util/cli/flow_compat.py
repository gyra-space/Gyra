...
_DEFAULT_PATH = os.path.join(
    ROOT_PATH, "packages", "gyra-serve", "src", "gyra_serve", "flow", "compat"
)
...
@click.option(
    ...
    required=False,
    help=_(
        "The module to scan, if not set, will scan all GYRA modules("
        "'gyra,gyra_client,gyra_ext,gyra_serve,gyra_app')."
    ),
)
@click.option(
    "--output",
    ...
    help=_(
        ...
        "packages/gyra-serve/src/gyra_serve/flow/compat/"
    ),
)
@click.option(
    "--curr_version",
    ...
    required=False,
    help=_(
        "The current version of the flow, if not set, will read from gyra.__version__"
    ),
)
@click.option(
