...
from gyra.configs.model_config import resolve_root_path, LOGDIR
...
def _get_logging_level() -> str:
    return os.getenv("GYRA_LOG_LEVEL", "INFO")
...
def _build_logger(
    ...
    def _build_handler(_config: LoggingParameters) -> logging.Handler:
        ...
        _handler = GyraTimedRotatingFileHandler(
            filename=os.path.join(
                LOGDIR, _config.file or "gyra_default.log"
            ),  # 日志文件名
    ...
    with _locker:
        ...
        if not logger_name:
            ...
            handlers_to_remove = [
                ...
                and not isinstance(h, GyraTimedRotatingFileHandler)
            ]
...
class GyraTimedRotatingFileHandler(TimedRotatingFileHandler):
