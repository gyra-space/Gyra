...
def create_app(*args, **kwargs) -> FastAPI:
    ...
    if _sp:
        app.__gyra_custom_lifespan = _sp
        print(f"[create_app] Set __gyra_custom_lifespan", file=sys.stderr, flush=True)
    return app
...
def replace_router(app: FastAPI, router: Optional[APIRouter] = None):
    ...
    if _FASTAPI_VERSION >= "0.109.1":
        if hasattr(app, "__gyra_custom_lifespan"):
            _sp = getattr(app, "__gyra_custom_lifespan")
            router.lifespan_context = _sp
            print(f"[replace_router] Set lifespan_context on router", file=sys.stderr, flush=True)
        else:
            print("[replace_router] No __gyra_custom_lifespan found", file=sys.stderr, flush=True)
