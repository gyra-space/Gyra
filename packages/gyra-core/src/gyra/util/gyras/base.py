...
DEFAULT_GYRAS_DIR = Path.home() / ".gyras"
GYRAS_HOME = os.getenv("GYRAS_HOME", str(DEFAULT_GYRAS_DIR))
GYRAS_REPO_HOME = os.getenv("GYRAS_REPO_HOME", str(DEFAULT_GYRAS_DIR / "repos"))

DEFAULT_REPO_MAP = {
    "eosphoros/gyras": "https://github.com/eosphoros-ai/gyras.git",
}
...
GYRAS_METADATA_FILE = "gyras.toml"
...
def get_repo_path(repo: str) -> str:
    ...
    return str(Path(GYRAS_REPO_HOME) / repo_group / repo_name)


ENV_SIG = _get_env_sig()
# The directory where the gyras package is installed
INSTALL_DIR = Path(GYRAS_HOME) / "packages" / ENV_SIG


os.makedirs(GYRAS_REPO_HOME, exist_ok=True)
