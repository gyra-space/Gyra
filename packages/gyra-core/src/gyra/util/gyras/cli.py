...
def add_tap_options(func):
    @click.option(
        ...
        default=None,
        required=False,
        help="The repository to install the gyras from",
    )
...
def install(repo: str | None, update: bool, names: list[str]):
    """Install your gyras(operators,agents,workflows or apps)"""
...
def uninstall(names: list[str]):
    """Uninstall your gyras(operators,agents,workflows or apps)"""
...
def reinstall(repo: str | None, update: bool, names: list[str]):
    """Reinstall your gyras(operators,agents,workflows or apps)"""
...
@click.option(
    ...
    type=str,
    required=True,
    help="The name you want to give to the gyra",
)
@click.option(
    "-l",
    "--label",
...
@click.option(
    ...
    default=None,
    required=False,
    help="The description of the gyra",
)
@click.option(
    "-t",
    "--type",
...
@click.option(
    ...
    default="json",
    required=False,
    help="The definition type of the gyra",
)
@click.option(
    "-C",
    "--directory",
    ...
    default=None,
    required=False,
    help="The working directory of the gyra(defaults to the current directory).",
)
def new_gyras(
    name: str,
    label: str | None,
    ...
    """New a gyras module structure"""
    if not label:
        # Set label to the name
        default_label = name.replace("-", " ").replace("_", " ").title()
        label = click.prompt(
            "Please input the label of the gyra", default=default_label
        )
    if not description:
        # Read with click
        description = click.prompt(
            "Please input the description of the gyra", default=""
        )
    if not directory:
        # Set directory to the current directory(abs path)
        directory = click.prompt(
            "Please input the working directory of the gyra",
