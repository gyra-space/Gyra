...
from .base import GYRAS_METADATA_FILE, TYPE_TO_PACKAGE
...
def create_template(
    ...
    working_directory = os.path.join(working_directory, TYPE_TO_PACKAGE[gyras_type])
    ...
    if gyras_type == "flow":
        ...
        raise ValueError(f"Invalid gyras type: {gyras_type}")
...
def _create_flow_template(
    ...
    _write_gyras_toml(working_directory, name, json_dict)
...
def _create_operator_template(
    ...
    if definition_type != "python":
        raise click.ClickException(
            f"Unsupported definition type: {definition_type} for gyras type: "
            f"{gyras_type}"
        )
    # Create project structure
    _create_project_structure(working_directory, name, base_metadata.get("description"))
...
def _write_gyras_toml(working_directory: str, name: str, json_data: dict):
    """Write the gyras.toml file"""

    import tomlkit

    with open(Path(working_directory) / name / GYRAS_METADATA_FILE, "w") as f:
...
def _write_operator_init_file(working_directory: str, name: str, mod_name: str):
    ...
    init_file = Path(working_directory) / name / mod_name / "__init__.py"
    content = """
from gyra.core.awel import MapOperator
from gyra.core.awel.flow import ViewMetadata, OperatorCategory, IOField, Parameter
...
from gyra.agent.util import cmp_string_equal
...
class HelloWorldSpeakerAgent(ConversableAgent):
    ...
    def init_reply_message(self, received_message: AgentMessage) -> AgentMessage:
        reply_message = super().init_reply_message(received_message)
...
async def _test_agent():
    ...
    \"\"\"
    from gyra.agent import AgentContext, AgentMemory, UserProxyAgent, LLMConfig
...
from gyra.agent.resource import tool
