...
from gyra._private.pydantic import BaseModel, ConfigDict, Field, model_validator
from gyra.component import BaseComponent, SystemApp
from gyra.core.awel import DAG
from gyra.core.awel.flow.flow_factory import FlowPanel
from gyra.util.gyras.base import (
    GYRAS_METADATA_FILE,
    INSTALL_DIR,
...
class BasePackage(BaseModel):
    ...
    def load_module_class(
        ...
        from gyra.core.awel.dag.loader import _load_modules_from_file
...
class FlowPythonPackage(FlowPackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from gyra.core.awel.dag.loader import _process_modules
...
class ResourcePackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from gyra.agent.resource import Resource
        from gyra.agent.resource.tool.pack import _is_function_tool
...
def parse_package_metadata(package: InstalledPackage) -> BasePackage:
    with open(
        Path(package.root) / GYRAS_METADATA_FILE, mode="r+", encoding="utf-8"
    ) as f:
...
def _load_installed_package(path: str) -> List[InstalledPackage]:
    ...
    for package in os.listdir(path):
        ...
        gyras_metadata_file = full_path / GYRAS_METADATA_FILE
        if (
            full_path.is_dir()
            and install_metadata_file.exists()
            and gyras_metadata_file.exists()
        ):
...
class GYRAsLoader(BaseComponent):
    """The loader of the gyras packages"""

    name: str = "gyra_gyras_loader"

    def __init__(
        self,
        system_app: Optional[SystemApp] = None,
        install_dir: Optional[str] = None,
        load_gyras_interval: int = 10,
    ):
        """Initialize the GYRAsLoader."""
        self._system_app = None
        self._install_dir = install_dir or INSTALL_DIR
        self._packages: Dict[str, BasePackage] = {}
        self._load_gyras_interval = load_gyras_interval
    ...
    def before_start(self):
        ...
        schedule.every(self._load_gyras_interval).seconds.do(self.load_package)
    ...
    def load_package(self, is_first: bool = False) -> None:
        ...
        try:
            ...
            if is_first:
                logger.info(
                    f"Found {len(packages)} gyras packages from {self._install_dir}"
                )
            for package in packages:
                self._packages[package.name] = package
                self._register_packages(package)
        except Exception as e:
            logger.warning(f"Load gyras package error: {e}", e)
    ...
    def _register_packages(self, package: BasePackage):
        if package.package_type == "agent":
            from gyra.agent import ConversableAgent, get_agent_manager
