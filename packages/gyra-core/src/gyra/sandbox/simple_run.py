...
from gyra.sandbox.base import SandboxBase, DEFAULT_WORK_DIR
...
async def main():
    ...
    from gyra_ext.sandbox.xic.xic_client import XICSandbox
    ...
    gyra_skill_dir = f"{xic_client.work_dir}/skills/gyra-knowledge"
    ...
    # resp1: ShellCommandResult = await xic_client.shell.exec_command(command=f"cd {DEFAULT_WORK_DIR} && cat .gyra")
    ...
    #
    # ## 恢复沙箱
    # re_sandbox = await XICSandbox.recovery(user_id="184089", agent="gyra", chat_session=chat_session_id,
    #                                                   template="9c85374e-1564-4313-be9f-3432f576b19e")
    #
    # ## 查看恢复文件
    ...
    ## 恢复沙箱a
    # re_sandbox = await XICSandbox.recovery(user_id="184089", agent="gyra",
