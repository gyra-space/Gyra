...
import asyncio
from gyra.vis import Signal, Effect, batch
from gyra.vis.parts import TextPart, CodePart, ToolUsePart, ThinkingPart, PlanPart
from gyra.vis.unified_converter import UnifiedVisConverter, UnifiedVisManager
from gyra.vis.decorators import vis_component, streaming_part, auto_vis_output
...
async def example_unified_converter():
    ...
    # 手动添加Part
    text_part = TextPart.create(content="欢迎来到Gyra!")
    converter.add_part_manually(text_part)
    
    code_part = CodePart.create(
        code="print('Hello, Gyra!')",
        language="python"
    )
