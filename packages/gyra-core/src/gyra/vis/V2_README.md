...
### 2. JSON Lines Converter (`gyra.vis.protocol.jsonlines`)

The JSON Lines format provides stream-friendly serialization.

```python
from gyra.vis.protocol.jsonlines import (
    VisJsonLinesBuilder,
    vis_builder,
)
...
### 3. Incremental Index Manager (`gyra.vis.index`)

Provides O(1) updates instead of O(n) full rebuilds.

```python
from gyra.vis.index import IncrementalIndexManager, IndexEntry
...
```python
from gyra.vis.vis_converter import VisProtocolConverter
from gyra.vis.protocol.jsonlines import VisJsonLinesConverter
...
```bash
# Backend tests
pytest packages/gyra-core/src/gyra/vis/*/tests/
...
- `gyra.vis.schema_v2` - Schema definitions
- `gyra.vis.index` - Incremental index manager
- `gyra.vis.protocol.jsonlines` - JSON Lines converter
