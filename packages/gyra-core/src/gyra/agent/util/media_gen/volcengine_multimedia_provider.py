"""Volcano Engine (火山) Multimedia Provider.

Merges image generation (Seedream / 豆包图像) and video generation
(Seedance / 豆包视频) under a single vendor protocol ``volcengine_multimedia``.
The caller routes by the model's ``model_type`` (image/video) to the right
method; this provider delegates to the corresponding specialized provider.
"""
...
from gyra.agent.util.media_gen.volcengine_audio_provider import VolcengineAudioProvider
from gyra.agent.util.media_gen.volcengine_image_provider import VolcengineImageProvider

logger = logging.getLogger(__name__)


@MediaGenProviderRegistry.register(
    protocol="volcengine_multimedia", env_key="ARK_API_KEY"
)
class VolcengineMultimediaProvider(MediaGenProvider):
    """火山多媒体 provider：图片走 Seedream，视频走 Seedance。"""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._image = VolcengineImageProvider(
            api_key=api_key, base_url=base_url, **kwargs
        )
        self._video = SeedanceVideoProvider(
            api_key=api_key, base_url=base_url, **kwargs
        )
        self._audio = VolcengineAudioProvider(
            api_key=api_key, base_url=base_url, **kwargs
        )
