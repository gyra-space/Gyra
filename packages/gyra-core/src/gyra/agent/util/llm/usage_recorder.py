...
gyra-core 侧只定义记录结构与注册表，不感知存储（避免 gyra-core -> gyra-serve 依赖）。
gyra-serve 在启动时通过 ``register_llm_usage_recorder`` 注册一个写 DB 的回调。
...
- 未注册 recorder 时（gyra-core 单跑 / 测试）``record_llm_usage`` 空转，零侵入。
"""
...
def register_llm_usage_recorder(
    ...
    """注册一个 async recorder 回调。gyra-serve 启动时调用。"""
