...
def get_tool_resource_manager():
    """获取工具资源管理器"""
    from gyra.agent.tools import tool_resource_manager
...
async def get_tools_by_category(
    ...
    用于前端工具选择组件的分类展示
    """
    from gyra.agent.tools.resource_manager import ToolVisibility, ToolStatus
...
async def list_all_tools(
    ...
    if query:
        ...
        from gyra.agent.tools.base import ToolSource
...
async def update_tool(request: dict):
    """更新工具配置"""
    from gyra.agent.tools.resource_manager import ToolStatus, ToolVisibility
...
def setup_tool_api(app):
    ...
    使用方式:
        from gyra.agent.tools.api_server import setup_tool_api
        setup_tool_api(app)
    """
