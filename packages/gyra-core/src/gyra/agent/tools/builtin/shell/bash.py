    ...
    def _define_metadata(self) -> ToolMetadata:
        return ToolMetadata(
            ...
            description=(
                ...
                "- Path fence: every path-like argument in the command must stay "
                "inside the sandbox workspace — including search roots "
                "(e.g. `find /` is rejected even if the target file is inside "
                "the workspace) and `cd` targets. Use relative paths anchored "
                "at the working directory (e.g. `find . -name x`) or absolute "
                "paths within the workspace; `~` and `..` are forbidden too. "
                "One out-of-workspace path rejects the entire command.\n\n"
