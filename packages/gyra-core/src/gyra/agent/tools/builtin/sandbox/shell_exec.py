...
- 路径栅栏（最常见被拒原因，务必遵守）：整条命令中**所有路径型参数**都必须位于沙箱工作空间内——包括 find/grep/ls 的搜索起点、cd 的目标等，即使目标文件在工作空间内也不例外（如 find / 会被拒绝）。请使用以当前工作目录为起点的相对路径（如 find . -name x），或工作空间内的绝对路径；~ 与 .. 越界同样禁止。任一参数越界，整条命令都会被拒绝执行。"""
