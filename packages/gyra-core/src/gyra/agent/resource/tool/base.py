...
此模块已迁移到统一工具框架。
所有工具类和装饰器现在从 gyra.agent.tools 导入。

旧的导入路径:
    from gyra.agent.resource.tool.base import tool, BaseTool, FunctionTool, ...

新的导入路径 (推荐):
    from gyra.agent.tools import tool, ToolBase, ToolRegistry, ...
...
from gyra._private.pydantic import BaseModel, Field, model_validator
from gyra.util.configure.base import _MISSING
...
# 装饰器
from gyra.agent.tools.decorators import (
    tool,
    gyra_tool,
    system_tool,
    sandbox_tool,
