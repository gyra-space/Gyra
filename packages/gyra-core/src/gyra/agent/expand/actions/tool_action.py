...
from gyra.eval.tool.llm_snapshot_tool import LLMSnapshotTool, ToolSnapshot
from gyra.eval.tool_run_model import RunModel, ToolRunModel
...
class UnifiedToolAdapter(BaseTool):
    ...
    将新框架的 ToolBase (gyra.agent.tools.base.ToolBase) 适配为
    ...
    def args(self):
        """工具参数 - 转换为旧格式"""
        from gyra.agent.resource.tool.base import ToolParameter
...
class ToolAction(Action[ToolInput]):
    ...
    async def run(
        ...
        **kwargs,
        ...
        if should_truncate:
            from gyra.agent.expand.react_master_agent.truncation import Truncator
    ...
    def _try_get_unified_tool(self, tool_name: str) -> Optional[BaseTool]:
        ...
        """
        try:
            from gyra.agent.tools import tool_registry
        ...
        try:
            if (
                ...
                if dispatcher is not None and snap is not None:
                    try:
                        from gyra.core.interface.resource.dispatcher import (
                            ToolDispatcher as _ToolDispatcher,
                        )
                        from gyra.core.interface.resource.tool_entry import (
