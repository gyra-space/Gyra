...
class MemoryReflectAgent(MemoryAgentBase):
    ...
    profile: ProfileConfig = ProfileConfig(
        name=DynConfig(
            "MemoryReflectAgent",
            category="agent",
            key="gyra_agent_expand_memory_reflect_agent_profile_name",
        ),
        role=DynConfig(
            "Memory Reflect Agent",
            category="agent",
            key="gyra_agent_expand_memory_reflect_agent_profile_role",
        ),
        goal=DynConfig(
            "Review recent turns and consolidate user profile, preferences, "
            "and decisions into long-term memory.",
            category="agent",
            key="gyra_agent_expand_memory_reflect_agent_profile_goal",
        ),
        desc=DynConfig(
            "Built-in memory agent: cross-turn reflection (tier 2).",
            category="agent",
            key="gyra_agent_expand_memory_reflect_agent_profile_desc",
        ),
