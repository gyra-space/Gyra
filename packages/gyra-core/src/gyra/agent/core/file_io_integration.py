...
- PDCA-style agents:
    from gyra.agent.core.file_io_integration import (
    ...
    try:
        ...
        if base_working_dir is None:
            try:
                ...
                base_working_dir = os.path.expanduser("~/.gyra/agent_storage")
...
async def process_user_files(
    ...
    try:
        ...
        if sandbox is None and system_app:
            try:
                ...
                local_upload_dir = os.path.expanduser(
                    f"~/.gyra/uploads/{conv_id or 'default'}"
                )
        ...
        logger.warning("gyra_serve.agent.file_io not available")
        return {
            "multimodal_contents": [],
            "sandbox_file_refs": [],
...
class FileIOContext:
        ...
        try:
            ...
            if self.sandbox and self.sandbox.file:
                ...
                if file_info and file_info.content:
                    from gyra.agent.core.memory.gpts import FileType as AfsFileType
    ...
    def render_deliveries(self, title: str = "Delivered Files") -> str:
        ...
        try:
            from gyra_serve.agent.file_io import render_dattach_list
