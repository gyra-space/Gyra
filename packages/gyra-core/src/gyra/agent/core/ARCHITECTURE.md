...
GYRA Core 是一个基于学术论文《A Survey on Large Language Model Based Autonomous Agents》设计的 Agent 框架。框架将 Agent 架构分为四个核心模块：
...
```
packages/gyra-core/src/gyra/agent/core/
...
```python
from gyra.agent.core import ConversableAgent, AgentInfo, AgentMode
...
```python
from gyra.agent.core.action.base import Action, ActionOutput
...
```python
from gyra.agent.core.skill import Skill, SkillMetadata, SkillType, skill
...
```python
from gyra.agent.core.memory.base import Memory, MemoryFragment
...
```python
# 系统工具
from gyra.agent.core.system_tool_registry import system_tool_dict
...
# 沙箱工具
from gyra.agent.core.sandbox.sandbox_tool_registry import sandbox_tool
...
```python
from gyra.agent.core.agent_info import PermissionRule, PermissionAction, PermissionRuleset
...
```python
from gyra.agent.core.execution_engine import ExecutionHooks
...
from gyra.agent.core import AgentInfo
...
```python
from gyra.agent.core.execution import (
...
```python
from gyra.agent.core.context_lifecycle import AgentContextIntegration
...
from gyra.agent.core.execution import create_execution_loop
from gyra.agent.core.context_lifecycle import AgentContextIntegration
...
```
packages/gyra-core/src/gyra/agent/
├── interaction/                    # 交互系统模块
│   ├── __init__.py
...
```python
from gyra.agent.core import ConversableAgent, InteractionAdapter
...
```python
from gyra.agent.shared import SharedSessionContext, SharedContextConfig
...
```python
from gyra.agent.shared import ContextArchiver, ContentType
...
```python
from gyra.agent.shared import TaskBoardManager, TaskStatus, TaskPriority
...
```python
from gyra.agent.shared import SharedSessionContext, V1ContextAdapter
