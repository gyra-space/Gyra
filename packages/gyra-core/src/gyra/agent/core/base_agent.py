...
from gyra._private.pydantic import ConfigDict, Field, PrivateAttr, BaseModel
from gyra.core import LLMClient, ModelMessageRoleType, PromptTemplate, HumanMessage
...
from gyra.util.logger import colored, digest
...
class ConversableAgent(Role, Agent):
    ...
    async def sandbox_tool_injection(self):
        ...
        if self.sandbox_manager:
            logger.info("注入沙箱工具！")
            from gyra.agent.core.sandbox.sandbox_tool_registry import (
                sandbox_tool_dict,
            )
    ...
    def _inject_database_tools(self):
        ...
        # 故在 serve 层)。失败不阻塞注入。
        try:
            from gyra_serve.agent.capabilities.db.tools import (
    ...
    def function_callning_reply_messages(
        ...
        from gyra.core import ModelMessageRoleType
    ...
    def register_variables(self):
        ...
        async def var_sandbox(instance):
            ...
            if instance and instance.sandbox_manager:
                ...
                from gyra.agent.core.sandbox.prompt import sandbox_prompt
    ...
    async def get_all_custom_variables(self) -> List[DynamicParam]:
        from gyra.agent.core.reasoning.reasoning_arg_supplier import (
    ...
    async def push_context_event(
        ...
        from gyra.context.utils import build_operator_config
        from gyra.context.operator import Operator, OperatorManager
