...
from gyra.util.executor_utils import blocking_func_to_async, execute_no_wait
from gyra.component import LifeCycle
...
class GptsMemory(LifeCycle, FileMetadataStorage, WorkLogStorage, KanbanStorage, TodoStorage):
    ...
    name = "gyra_gpts_memory"  # Component name for registration
    ...
    async def _load_event_log(self, conv_id: str) -> list:
        ...
        Lazy import 避免循环依赖。如果 gyra_serve 不可用（如单元测试），
        返回空列表。
        """
        try:
