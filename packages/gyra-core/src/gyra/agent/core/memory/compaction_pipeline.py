...
class UnifiedCompactionPipeline:
    ...
    async def compact_if_needed(
        ...
        if self.work_log_storage and chapter:
            try:
                from gyra.agent.core.memory.gpts.file_base import WorkLogSummary
    ...
    async def _generate_llm_summary(
        ...
        try:
            ...
            from gyra.agent.util.llm.call_utils import call_llm
    ...
    def _create_summary_message(
        ...
        if chapter.skill_outputs:
            ...
            if total_skills <= 3:
                ...
                parts.append(f"\n[{evicted_count} skill output(s) evicted to save context, use Skill to reload]")
                # Last 2 skills (full)
