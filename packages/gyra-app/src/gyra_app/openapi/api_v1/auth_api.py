...
from gyra_app.auth.oauth import OAuth2Service
from gyra_app.auth.session import (
    SessionManager,
...
async def oauth_callback(
    ...
    logger.info(f"[local_login] Cookie set: gyra_session, host={host}, scheme={request.url.scheme}, secure={request.url.scheme == 'https'}")
    return response
...
async def get_current_user(request: Request):
    ...
    token = request.cookies.get("gyra_session") or request.headers.get(
        "Authorization", ""
    ).replace("Bearer ", "")
    if not token:
...
async def logout(request: Request):
    ...
    response.delete_cookie("gyra_session")
    return response
