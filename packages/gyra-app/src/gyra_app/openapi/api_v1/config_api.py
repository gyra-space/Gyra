...
from gyra_core.config.schema import AppConfig
...
def _invalidate_embedding_caches() -> None:
    ...
    try:
        ...
        storage_manager = system_app.get_component(
            "gyra_rag_storage_manager", StorageManager, default_component=None
        )
...
def register_embedding_model(
    ...
    *,
    ...
    3. Persist it to gyra.json so it survives a restart.
    """
    from gyra_app.initialization.embedding_component import get_embedding_registry
    ...
    try:
        from gyra_core.config.schema import EmbeddingModelConfig
...
async def create_agent(
    ...
    try:
        ...
        from gyra_core.config import AgentConfig, PermissionConfig
...
async def validate_config():
    ...
    try:
        ...
        from gyra_core.config import ConfigValidator
...
async def set_default_embedding_model(
    ...
    更新运行时 registry 并持久化到 gyra.json，同时使 RAG/记忆缓存失效。
    """
    try:
        from gyra_app.initialization.embedding_component import (
            get_embedding_registry,
...
async def update_oauth2_config(oauth2_data: Dict[str, Any]):
    ...
    try:
        from gyra_core.config import AppConfig, OAuth2Config
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Database error: {str(e)}. Please run: gyra db migration upgrade",
            )
...
async def get_feature_plugins_catalog():
    """Builtin plugin catalog merged with current enabled/settings from database or gyra.json."""
    from gyra_app.feature_plugins.catalog import merge_catalog_with_state
...
async def update_feature_plugins(
    ...
    from gyra_app.feature_plugins.catalog import is_known_plugin, get_manifest
    from gyra_app.feature_plugins.system_config_dao import SystemConfigDao
    from gyra_core.config import FeaturePluginEntry
...
async def update_file_service_config(request: FileServiceRequest):
    try:
        ...
        if request.backends:
            from gyra_core.config import FileBackendConfig
...
async def list_secrets():
    from gyra_core.config.encryption import list_secrets as get_secrets_status
...
async def set_secret(request: SecretRequest):
    from gyra_core.config.encryption import set_secret as save_secret_value
...
async def delete_secret(secret_name: str):
    from gyra_core.config.encryption import delete_secret as delete_secret_value
...
async def export_config_safe():
    from gyra_core.config.encryption import mask_secrets_in_json
...
async def set_llm_key(request: LLMKeyRequest):
    ...
    """
    from gyra_core.config.encryption import set_secret
    import logging
...
async def delete_llm_key(provider: str):
    ...
    """
    from gyra_core.config.encryption import delete_secret
...
def _apply_database_config(web_params, db_dict: Dict[str, Any]) -> None:
    ...
    """
    from gyra_core.config.schema import DatabaseConfig
