...
async def batch_execute(request: BatchExecuteRequest):
    """批量并行执行工具"""
    try:
        from gyra_core.tools import BatchExecutor
...
async def check_tool_permission(request: PermissionCheckRequest):
    """检查工具执行权限"""
    try:
        from gyra_core.permission import PermissionChecker, PRIMARY_PERMISSION
...
async def get_sandbox_status():
    """获取沙箱状态"""
    try:
        from gyra_core.sandbox import SandboxFactory
