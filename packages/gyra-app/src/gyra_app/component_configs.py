...
from gyra.vis.vis_manage import initialize_vis_convert
from gyra_app.config import ApplicationConfig, ServiceWebParameters

# TODO: rewire to new knowledge module (Task #9)
try:
    from gyra_serve.agent.resource.knowledge_pack import KnowledgePackSearchResource  # type: ignore
except ImportError:  # pragma: no cover - rag module removed
    KnowledgePackSearchResource = None  # type: ignore[assignment]

from gyra_serve.agent.resource.tool.local_tool import LocalToolPack
from gyra_serve.agent.resource.tool.mcp import MCPSSEToolPack
from gyra_serve.agent.resource.tool.mcp_collect import MCPCollectSSEToolPack
from gyra_serve.agent.resource.gyra_skill import GyraSkillResource

try:
    from gyra_serve.rag.storage_manager import StorageManager  # type: ignore
except ImportError:  # pragma: no cover - rag module removed
...
def initialize_components(
    ...
    from gyra_app.initialization.embedding_component import (
        _initialize_embedding_model,
        _initialize_rerank_model,
    )
    from gyra_app.initialization.scheduler import DefaultScheduler
    from gyra_app.initialization.serve_initialization import register_serve_apps
    from gyra_serve.datasource.manages.connector_manager import ConnectorManager
    from gyra.sandbox import initialize_sandbox_adapter
    ...
    # StorageManager was removed along with gyra_serve.rag; skip registration
...
def _initialize_model_cache(system_app: SystemApp, web_config: ServiceWebParameters):
    from gyra.storage.cache import initialize_cache
...
def _initialize_awel(system_app: SystemApp, awel_dirs: Optional[str] = None):
    from gyra.configs.model_config import _DAG_DEFINITION_DIR
    from gyra.core.awel import initialize_awel
...
def _initialize_agent(system_app: SystemApp):
    from gyra.agent import initialize_agent
...
def _initialize_resource_manager(system_app: SystemApp):
    from gyra.agent.expand.resources.gyra_tool import list_gyra_support_models
    from gyra.agent.resource.base import ResourceType
    from gyra.agent.resource.manage import get_resource_manager, initialize_resource, _SYSTEM_APP as check_system_app
    from gyra_serve.agent.resource.app import GptAppResource
    from gyra_serve.agent.resource.datasource import DatasourceResource
    ...
    from gyra_ext.agent.agents.open_ta.tools.xls_analysis import get_data_introduction
    from gyra_ext.agent.agents.open_ta.tools.xls_analysis import run_sql_with_file
    ...
    from gyra_ext.agent.agents.open_ta.tools.flamegraph_cpu_analyzer import (
        flamegraph_overview,
    )
    from gyra_ext.agent.agents.open_ta.tools.flamegraph_cpu_analyzer import (
    ...
    # from gyra_ext.agent.agents.smartTestUI.tool.image_find_static_bug_tools import find_image_bugs
    # from gyra_ext.agent.agents.smartTestUI.tool.test_analysis_tools import generate_analysis_tool
    # from gyra_ext.agent.agents.smartTestUI.tool.tvp_cypress_code_tools import tvp_cypress_code
    # from gyra_ext.agent.agents.smartTestUI.tool.tvp_run_task_tools import tvp_execute_case
    # from gyra_ext.agent.agents.smartTestUI.tool.tvp_url_tools import tvp_main_screenshot, tvp_multi_screenshot
    # from gyra_ext.agent.agents.smartTestUI.tool.ding_information_tools import ding_send_text
    # from gyra_ext.agent.agents.smartTestUI.tool.image_analysis_tools import analysis_image
    ...
    # from gyra_ext.agent.agents.smartTestUI.tool.risk_punish_tools import create_dima_link
    # rm.register_resource(resource_instance=create_dima_link)

    # Register scene generate tool
    # from gyra_ext.agent.agents.smartTestFlux.tool.flux_scene_generate_tools import query_interface_description
    # from gyra_ext.agent.agents.smartTestFlux.tool.flux_scene_generate_tools import select_important_feature_key
    # from gyra_ext.agent.agents.smartTestFlux.tool.flux_scene_generate_tools import select_important_feature_key_value_list
    # from gyra_ext.agent.agents.smartTestFlux.tool.flux_scene_generate_tools import pair_feature
...
def _initialize_oracle_thick_mode(param: ApplicationConfig = None):
    ...
    2. gyra.json file: datasource.oracle_enable_thick_mode
    ...
    # 2. Check gyra.json file (main config source)
    if not enable_thick_mode:
        try:
            from gyra_core.config import ConfigManager, ConfigLoader
            ...
            gyra_home = get_gyra_home()
            logger.info(f"[OracleInit] GYRA_HOME={gyra_home}")
            ...
            if config:
                ...
                if datasource_config:
                    ...
                    if file_enable:
                        enable_thick_mode = True
                        logger.info("[OracleInit] gyra.json oracle_enable_thick_mode=true detected")
            ...
            logger.warning(f"[OracleInit] Failed to read gyra.json config: {e}")
            import traceback
    ...
    if not enable_thick_mode:
        try:
            from gyra_serve.config.service.service import get_config_by_key
    ...
    if not enable_thick_mode:
        logger.info(
            ...
            "Enable via gyra.json (datasource.oracle_enable_thick_mode=true) "
            "or set ORACLE_ENABLE_THICK_MODE=true for Oracle 11g support."
        )
        return

    try:
        from gyra_ext.datasource.rdbms.conn_oracle import _init_thick_mode
        ...
        if success:
            ...
            logger.warning(
                ...
                "in gyra.json or ORACLE_INSTANT_CLIENT_HOME environment variable."
            )
    except ImportError:
        logger.debug("[OracleInit] gyra_ext not available, skipping Oracle thick mode init")
    except Exception as e:
...
def _initialize_operators():
    from gyra_app.operators.code import CodeMapOperator  # noqa: F401
    from gyra_app.operators.converter import StringToInteger  # noqa: F401

    from gyra_app.operators.llm import (  # noqa: F401
        HOLLMOperator,
        HOStreamingLLMOperator,
    )
    # TODO: rewire to new knowledge module (Task #9)
    # from gyra_app.operators.rag import HOKnowledgeOperator  # noqa: F401
    from gyra_serve.agent.resource.datasource import DatasourceResource  # noqa: F401


def _initialize_code_server(system_app: SystemApp):
    from gyra.util.code.server import initialize_code_server

    initialize_code_server(system_app)


def _initialize_local_tool(system_app: SystemApp):
    from gyra_serve.agent.resource.func_registry import central_registry
...
def _initialize_context(system_app: SystemApp):
    from gyra.context.operator import OperatorManager

    OperatorManager.operator_scan()


def _initialize_perfermance(system_app: SystemApp):
    from gyra.perf.profiler import PerformanceProfiler
