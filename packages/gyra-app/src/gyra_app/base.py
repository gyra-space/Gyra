...
from gyra.core.interface.parameter import BaseServerParameters
from gyra.datasource.parameter import BaseDatasourceParameters
from gyra.datasource.rdbms.base import RDBMSConnector, RDBMSDatasourceParameters
from gyra_app.config import ApplicationConfig, ServiceConfig
...
def _initialize_db_storage(param: ServiceConfig, system_app: SystemApp):
    ...
    if isinstance(db_config, SQLiteConnectorParameters):
        ...
        raise ValueError(
            "GYRA only support SQLite, MySQL and OceanBase database as metadata "
            "storage database"
        )
...
def _migrate_mysql_chat_tables_utf8mb4(db_url: str) -> None:
    ...
    New installs get correct DDL from assets/schema/gyra.sql. Legacy MySQL
...
def _migration_db_storage(
    ...
    from gyra.configs.model_config import PILOT_PATH
    from gyra_app.initialization.db_model_initialization import _MODELS  # noqa: F401
    from gyra_ext.datasource.rdbms.conn_sqlite import SQLiteConnectorParameters

    default_meta_data_path = os.path.join(PILOT_PATH, "meta_data")
    if not disable_alembic_upgrade:
        from gyra.storage.metadata.db_manager import db
        from gyra.util._db_migration_utils import _ddl_init_and_upgrade
        ...
        if (
            ...
            init and upgrade. "
                "1.If you are use GYRA firstly, please manually execute the following\
                 command to initialize, 
                `mysql -h127.0.0.1 -uroot -p{your_password} \
                < ./assets/schema/gyra.sql` "
                "2.If there are any changes to the table columns in the GYRA database,
...
def _initialize_db(
    ...
    from gyra.configs.model_config import PILOT_PATH
    from gyra.storage.metadata.db_manager import initialize_db
    ...
    if system_app:
        from gyra.storage.metadata import UnifiedDBManagerFactory
...
class WebServerParameters(BaseServerParameters):
    ...
    remote_embedding: Optional[bool] = field(
        ...
        metadata={
            ...
            " to start a embedding model through `gyra start worker --worker_type "
    ...
    remote_rerank: Optional[bool] = field(
        ...
        metadata={
            ...
            " to start a rerank model through `gyra start worker --worker_type "
    ...
    log_file: Optional[str] = field(
        default="gyra_webserver.log",
        metadata={
            "help": "The filename to store log",
        },
    )
    tracer_file: Optional[str] = field(
        default="gyra_webserver_tracer.jsonl",
        metadata={
