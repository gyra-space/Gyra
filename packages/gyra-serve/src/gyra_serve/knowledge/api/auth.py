...
Follows the existing gyra-serve conventions instead of inventing a new
system:

- **User identity**: `gyra_serve.utils.auth.get_user_from_headers`
  (X-User-ID header / gyra_session cookie / JWT). When the permissions
...
- **API keys**: the `check_api_key` pattern from
  `gyra_serve.datasource.api.endpoints` — when `ServeConfig.api_keys`
...
def check_space_access(space: Any, user: UserRequest, write: bool) -> None:
    ...
    (a `gyra.knowledge.types.Space` or a list_spaces() dict).
    """
