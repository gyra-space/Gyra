...
from gyra.util.gyras.repo import _install_default_repos_if_no_repos, list_gyras
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    """The service class for GyrasHub"""
    ...
    def get_list(self, request: ServeRequest) -> List[ServerResponse]:
        """Get a list of GyrasHub entities
    ...
    def get_list_by_page(
        ...
        """Get a list of GyrasHub entities by page
        ...
        return self.dao.gyras_list(query_request, page, page_size)
    ...
    def refresh_hub_from_git(
        ...
        data: List[Tuple[str, str, str, str]] = list_gyras()

        from gyra.util.gyras.loader import (
            BasePackage,
            InstalledPackage,
        ...
        try:
            for repo, package, name, gpts_path in data:
                try:
                    if not name:
                        logger.info(
                            f"gyras error repo:{repo}, package:{package}, name:{name},"
                            f" gpts_path:{gpts_path}"
                        )
                        continue
                    ...
                    if old_hub_info:
                        ...
                        request.author = self._get_gyras_author(base_package.authors)
                        request.email = self._get_gyras_email(base_package.authors)
                    ...
                    logger.warning(
                        f"Load from git failed repo:{repo}, package:{package}, "
                        f"name:{name}, gpts_path:{gpts_path}",
                        e,
                    )
    ...
    def _get_gyras_author(self, authors):
        pattern = r"(.+?)<"
        names = []
        for item in authors:
    ...
    def _get_gyras_email(self, authors):
        pattern = r"<(.*?)>"
        emails: List[str] = []
        for item in authors:
