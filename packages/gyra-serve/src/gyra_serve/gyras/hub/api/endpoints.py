...
from gyra_serve.core import Result, blocking_func_to_async
...
async def source_refresh(
    ...
    try:
        ...
        logger.error("Gyras hub source refresh Error!", e)
        return Result.failed(err_code="E0020", msg=f"Gyras Hub refresh Error! {e}")


@router.post("/install", response_model=Result[str])
async def install(request: ServeRequest):
    logger.info(f"gyras install:{request.name},{request.type}")

    try:
        from gyra_serve.gyras.my.config import (
            SERVE_SERVICE_COMPONENT_NAME as MY_GPTS_SERVICE_COMPONENT,
        )
        from gyra_serve.gyras.my.service.service import Service as MyGptsService
