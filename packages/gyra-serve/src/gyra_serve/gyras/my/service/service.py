...
from gyra.util.gyras.base import INSTALL_DIR
from gyra.util.gyras.repo import (
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def get_list(self, request: ServeRequest) -> List[ServerResponse]:
        """Get a list of GyrasMy entities
    ...
    def get_list_by_page(
        ...
        """Get a list of GyrasMy entities by page
    ...
    def install_gpts(
        ...
        try:
            ...
            inner_copy_and_install(repo, name, Path(gyra_path))
        except Exception as e:
            logger.exception(f"install_gpts failed!{str(e)}")
            raise ValueError(f"Install gyras [{type}:{name}] Failed! {str(e)}", e)
        ...
        gyras_entity = self.get(ServeRequest(name=name, type=type))
        ...
        if not gyras_entity:
            ...
            gyras_entity.version = base_package.version

            return self.update(ServeRequest(**gyras_entity.to_dict()))
    ...
    def uninstall_gpts(
        ...
        try:
            ...
            logger.warning(f"Uninstall gyras [{type}:{name}] Failed! {str(e)}", e)
            raise ValueError(f"Uninstall gyras [{type}:{name}] Failed! {str(e)}", e)
