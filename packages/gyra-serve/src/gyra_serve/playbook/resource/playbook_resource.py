...
def _get_playbook_info(config: PlaybookConfig) -> Dict[str, Any]:
    """获取当前剧本的基本信息。"""
    return {
...
def _get_playbook_text_content(config: PlaybookConfig) -> Dict[str, Any]:
    """获取剧本的独立文本部分。"""
    return {
        "playbook_id": config.playbook_id,
        "playbook_name": config.playbook_name,
...
def _get_playbook_skills(config: PlaybookConfig) -> Dict[str, Any]:
    """获取剧本定义的技能列表。"""
    return {
        "playbook_id": config.playbook_id,
        "playbook_name": config.playbook_name,
...
def _get_playbook_resources(config: PlaybookConfig) -> Dict[str, Any]:
    """获取剧本定义的资源列表。"""
    return {
        "playbook_id": config.playbook_id,
        "playbook_name": config.playbook_name,
...
def _get_playbook_deliverables(config: PlaybookConfig) -> Dict[str, Any]:
    """获取剧本定义的产出物列表。"""
    return {
        "playbook_id": config.playbook_id,
        "playbook_name": config.playbook_name,
