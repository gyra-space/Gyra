...
3. 提案必须带回溯源(确认人要核对原始 SQL):
   miss_ref={{{{kind, pattern, datasource_id}}}}(取自该聚类), origin_sql=[该聚类的 example_sql]
4. 你成功为某个聚类提案后,用 mark_miss_learned(clusters=[{{kind, datasource_id, pattern}}], workspace_id={ws})
   把这些聚类标记为已学习,避免每日重复曝光
5. 已有概念不要重复提案;没有值得提案的内容就直接结束
6. 提案只进收件箱,不影响任何查询(人工 confirm 后生效)"""
