...
from gyra._private.pydantic import model_to_dict
...
from gyra_serve.core import BaseService, ResourceTypes
from gyra_serve.datasource.manages import ConnectorManager
...
class Service(
    ...
    def _recover_stale_tasks(self):
        ...
        from gyra_serve.datasource.manages.learning_task_db import DbLearningTaskDao
    ...
    def file_learning(self):
        ...
        if not self._file_learning:
            from gyra_serve.datasource.file_learning.service import FileLearningService
    ...
    def delete(self, datasource_id: str) -> Optional[DatasourceServeResponse]:
        ...
        if db_config:
            ...
            # Cascade: drop ECP asset refs pointing at this datasource and mark
            # the semantic objects bound to it as deprecated, so the ECP asset
            # list doesn't keep showing a deleted DB and its proposals/confirmed
            # objects stop being executable. ECP owns only the reference, so this
            # is the correct place to clean it up.
            try:
                from gyra_serve.ecp.models.models import AssetRefDao, SemanticObjectDao

                AssetRefDao().delete_by_ref_all_workspaces(
                    kind="db", ref_id=str(ds_id)
                )
                SemanticObjectDao().deprecate_by_datasource(ds_id)
            except Exception:  # noqa: BLE001
                logger.warning(
                    "cascade ECP cleanup for datasource %s failed",
                    ds_id,
                    exc_info=True,
                )
    ...
    def upload_db_file(self, file) -> Dict[str, str]:
        ...
        Saves the file to ~/.cache/gyra/datasource/ with a unique name
        ...
        storage_dir = str(Path.home() / ".cache" / "gyra" / "datasource")
