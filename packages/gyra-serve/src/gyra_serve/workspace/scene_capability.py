...
- **继承** ``gyra.core.interface.resource.capability.Capability``(ABC,需实现
...
注册:type_key="workspace_scene",由 ``gyra_serve.agent.capabilities.workspace_scene``
子包的 ``register_capability_to(registry)`` 桥接(CapabilityFactoryRegistry.discover
扫 gyra_serve.agent.capabilities.* 子包)。
"""
