...
def _generate_gyras_zip(package_name: str, flow: ServeRequest) -> io.BytesIO:
    ...
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        ...
        # Create gyras.toml
        gyras_toml = tomlkit.document()
        # Add flow information
        gyras_flow_toml = tomlkit.document()
        gyras_flow_toml.add("label", flow_label)
        name_with_comment = tomlkit.string(flow_name)
        name_with_comment.comment("A unique name for all gyras")
        gyras_flow_toml.add("name", name_with_comment)
        gyras_flow_toml.add("version", "0.1.0")
        gyras_flow_toml.add(
            "description",
            flow_description,
        )
        gyras_flow_toml.add("authors", [])
        ...
        gyras_flow_toml.add("definition_type", definition_type_with_comment)
        gyras_toml.add("flow", gyras_flow_toml)

        # Add python and json config
        python_config = tomlkit.table()
        gyras_toml.add("python_config", python_config)
        json_config = tomlkit.table()
        json_config.add("file_path", "definition/flow_definition.json")
        json_config.comment("Json config")
        gyras_toml.add("json_config", json_config)

        # Transform gyras.toml to string
        toml_string = tomlkit.dumps(gyras_toml)
        zip_file.writestr(f"{package_name}/gyras.toml", toml_string)
        ...
        project_section.add("description", flow_description or "A gyras package")
...
async def _parse_flow_from_zip_file(
    ...
    from gyra.util.gyras.loader import _load_flow_package_from_zip_path
