...
from gyra._private.pydantic import model_to_json
from gyra.component import SystemApp
from gyra.core.awel import DAG, BaseOperator, CommonLLMHttpRequestBody
from gyra.core.awel.flow.flow_factory import (
    FlowCategory,
    FlowFactory,
    State,
    fill_flow_panel,
)
from gyra.core.awel.trigger.http_trigger import CommonLLMHttpTrigger
from gyra.core.awel.util.chat_util import (
    is_chat_flow_type,
    safe_chat_stream_with_dag_task,
    safe_chat_with_dag_task,
)
from gyra.core.interface.llm import ModelOutput
...
from gyra.storage.metadata._base_dao import QUERY_SPEC
from gyra.util.gyras.loader import GYRAsLoader
from gyra.util.module_utils import model_scan
from gyra.util.pagination_utils import PaginationResult
from gyra_serve.core import BaseService, blocking_func_to_async
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def __init__(
        ...
        self._gyras_loader: Optional[GYRAsLoader] = None
    ...
    def init_app(self, system_app: SystemApp) -> None:
        ...
        self._gyras_loader = system_app.get_component(
            GYRAsLoader.name,
            GYRAsLoader,
            or_register_component=GYRAsLoader,
            load_gyras_interval=self._serve_config.load_gyras_interval,
        )
    ...
    def before_start(self):
        ...
        self._pre_load_dag_from_gyras()

    def after_start(self):
        """Execute after the application starts"""
        for path in [
            "gyra_serve.memory.operators.longterm_memory_operator",
        ]:
            model_scan(module_path=path, base_class=BaseOperator)

        self.load_dag_from_db()
        self.load_dag_from_gyras(is_first_load=True)
        schedule.every(self._serve_config.load_gyras_interval).seconds.do(
            self.load_dag_from_gyras
        )
    ...
    @property
    def gyras_loader(self) -> GYRAsLoader:
        """Returns the internal GYRAsLoader."""
        if self._gyras_loader is None:
            raise ValueError("GYRAsLoader is not initialized")
        return self._gyras_loader
    ...
    def _pre_load_dag_from_gyras(self):
        """Pre load DAG from gyras"""
        flows = self.gyras_loader.get_flows()
        for flow in flows:
            try:
    ...
    def load_dag_from_gyras(self, is_first_load: bool = False):
        """Load DAG from gyras"""
        flows = self.gyras_loader.get_flows()
        for flow in flows:
            try:
                ...
                logger.warning(
                    f"Load DAG {flow.name} from gyras error: {str(e)}, detail: "
                    f"{message}"
                )
    ...
    def _parse_flow_category(self, dag: DAG) -> FlowCategory:
        ...
        """
        from gyra.core.awel.flow.base import _get_type_cls
    ...
    async def debug_flow(
        ...
        Returns:
            AsyncIterator[ModelOutput]: The output
        """
        from gyra.core.awel.dag.dag_manager import _parse_metadata
    ...
    async def get_flow_files(self, flow_uid: str):
        ...
        if not flow:
            ...
            return None
        package = self.gyras_loader.get_flow_package(flow.name)
        if package:
            pkg_path = (
