...
from gyra.core.awel.flow.flow_factory import FlowCategory
...
async def export_flow(
    uid: str,
    export_type: Literal["json", "gyras"] = Query(
        "json", description="export type(json or gyras)"
    ),
    ...
    elif export_type == "gyras":
        from ..service.share_utils import _generate_gyras_zip

        if format == "json":
            raise HTTPException(
                status_code=400, detail="json response is not supported for gyras"
            )

        zip_buffer = await blocking_func_to_async(
            global_system_app, _generate_gyras_zip, package_name, flow
        )
        return StreamingResponse(
            zip_buffer,
