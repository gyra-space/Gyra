...
from gyra.core.awel.flow import IOField, OperatorCategory, ViewMetadata
from gyra.core.operators import PreChatHistoryLoadOperator
...
class ServePreChatHistoryLoadOperator(PreChatHistoryLoadOperator):
    """Pre-chat history load operator for GYRA serve component
    ...
    Args:
        ...
        message_storage (Optional[StorageInterface[MessageStorageItem, Any]], optional):
            The message storage, store the messages of one conversation. Defaults to
            None.

    If the storage or message_storage is not None, the storage or message_storage will
    be used first.
    ...
    If we can't get the storage, we will use the InMemoryStorage as the storage or
    message_storage.
    """
...
class DefaultServePreChatHistoryLoadOperator(ServePreChatHistoryLoadOperator):
    """Default pre-chat history load operator for GYRA serve component
    ...
    metadata = ViewMetadata(
        ...
        description=_(
            ...
            "It is the default storage of GYRA"
        ),
        parameters=[],
        inputs=[
            IOField.build_from(
