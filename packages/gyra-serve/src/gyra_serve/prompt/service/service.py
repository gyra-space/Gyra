...
from gyra.core import ModelMetadata, ModelRequest, PromptTemplate
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def get_type_targets(self, prompt_type: str):
        ...
        if type == PromptType.AGENT:
            ...
            from gyra_app.scene import ChatScene

            return [
                {"name": item.value(), "desc": item.describe()} for item in ChatScene
            ]
        elif type == PromptType.EVALUATE:
            from gyra.rag.evaluation.answer import LLMEvaluationMetric
    ...
    def load_template(
        ...
        if type == PromptType.AGENT:
            ...
            cfg = Config()
            from gyra_app.scene import AppScenePromptTemplateAdapter
            ...
            if not target:
                raise ValueError("请选择一个场景用来加载模版")
            try:
                from gyra.rag.evaluation.answer import (
                    LLMEvaluationMetric,
                )
    ...
    def verify_response(
        ...
        if type == PromptType.AGENT:
            try:
                if not target:
                    raise ValueError("请选择一个Agent用来加载模版")
                from gyra.agent.core import agent_manage
