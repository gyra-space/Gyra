...
from gyra_serve.core.tests.conftest import system_app  # noqa: F401
    ...
    [
        {
            "app_config": {
                "DEBUG": True,
                "gyra.serve.prompt.default_user": "gyra",
                "gyra.serve.prompt.default_sys_code": "gyra",
            }
        }
    ],
    indirect=True,
...
def test_config_default_user(service: Service):
    system_app = service._system_app
    assert system_app.config.get("DEBUG") is True
    assert system_app.config.get("gyra.serve.prompt.default_user") == "gyra"
    assert service.config is not None
    assert service.config.default_user == "gyra"
    assert service.config.default_sys_code == "gyra"
...
def test_service_create(service: Service, default_entity_dict):
    ...
    with db.session() as session:
        ...
        assert db_entity.sys_code == "gyra"
...
def test_service_get_list(service: Service):
    for i in range(3):
        service.create(
            ServeRequest(**{"prompt_name": f"prompt_{i}", "sys_code": "gyra"})
        )
    entities: List[ServerResponse] = service.get_list(ServeRequest(sys_code="gyra"))
    assert len(entities) == 3
    for i, entity in enumerate(entities):
        assert entity.sys_code == "gyra"
