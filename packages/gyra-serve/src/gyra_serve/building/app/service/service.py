...
from gyra.agent import get_agent_manager, AgentResource, ResourceType
from gyra.agent.core.plan.base import SingleAgentContext
from gyra.agent.core.plan.react.team_react_plan import AutoTeamContext
from gyra.context.utils import build_by_agent_config
from gyra.component import SystemApp
from gyra.storage.metadata import BaseDao
from gyra.util.pagination_utils import PaginationResult
from gyra.vis.schema import ChatLayout
from gyra.vis.vis_manage import get_vis_manager
from gyra_serve.agent.model import NativeTeamContext
...
# Hook names produced by gyra.agent.core.memory.hook_dispatcher.default_memory_hooks.
...
def _inject_memory_hooks(team_context_dict: Dict[str, Any], config_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    from gyra.agent.core.memory.hook_dispatcher import default_memory_hooks
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def create(self, request: ServeRequest) -> Optional[ServerResponse]:
        ...
        if not request.layout:
            from gyra_ext.vis.gyra.gyra_vis_incr_converter import (
                GyraVisIncrConverter,
            )

            defualt_convert = GyraVisIncrConverter()
        ...
        if not request.llm_config:
            from gyra.agent import LLMStrategyType
    ...
    def app_info_to_config(self, request: ServeRequest) -> AppConfigRequest:
        ...
        if is_v2:
            from gyra.agent.core.plan.unified_context import UnifiedTeamContext
    ...
    def _default_memory_embedding_model() -> Optional[str]:
        ...
        """
        try:
            from gyra_serve.rag.service.service import Service as RagService
    ...
    async def enable_memory(self, app_code: str) -> Dict[str, Any]:
        ...
        if existing_memories:
            ...
            # knowledge-vault system. The old `gyra_serve.rag` module has
    ...
    def _convert_llm_config_to_resource(self, request: ServeRequest) -> ServeRequest:
        ...
        if request.llm_config and isinstance(request.llm_config, dict):
            from gyra_serve.building.config.api.schemas import LLMResource
    ...
    async def load_define_app(self):
        ...
        系统启动时自动初始化 gyra_app_define 目录下的默认应用。
        ...
        # 2. 构建目标文件夹路径
        target_folder = os.path.join(current_dir, "gyra_app_define")
    ...
    async def list_hot_apps(self, query: GptsAppQuery):
        from gyra.storage.chat_history.chat_history_db import ChatHistoryDao
