...
async def create(
    ...
    """Create a new Building/recommendQuestion entity
...
async def query(
    ...
    """Query Building/recommendQuestion entities
...
@router.post("/update")
async def update(
    recommend_question: ServeRequest,
    service: Service = Depends(get_service),
    user_info: UserRequest = Depends(get_user_from_headers),
):
    try:
        ...
        service.update(recommend_question)
        return Result.succ([])
    except Exception as ex:
...
async def delete(
    ...
    try:
        service.delete(recommend_question)
        return Result.succ([])
    except Exception as ex:
        return Result.failed(err_code="E000X", msg=f"delete question error: {ex}")
