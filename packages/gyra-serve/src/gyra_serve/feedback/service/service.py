...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    ...
    def create_or_update(self, request: ServeRequest) -> ServerResponse:
        """
        First check whether the current user has likes, and if so, check whether it's
        consistent with the previous likes

        If it is inconsistent, delete the previous likes and create a new like;

        if it is consistent, an error will be reported and the likes already exist.
        Please do not like repeatedly
        """
        feedbacks = self.dao.get_list(
            ServeRequest(
        ...
        if len(feedbacks) > 1:
            raise Exception("current conversation message has more than one feedback.")
        if len(feedbacks) == 1:
            fb = feedbacks[0]
            if fb.feedback_type == request.feedback_type:
                raise Exception("Please do not repeat feedback")
