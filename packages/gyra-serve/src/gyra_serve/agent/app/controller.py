...
from gyra_serve.agent.app.gpts_server import available_llms
from gyra_serve.agent.db.gpts_app import (
    GptsApp,
    GptsAppDao,
    TransferSseRequest,
)
from gyra_serve.agent.db.gpts_tool import (
    GptsTool,
    GptsToolDao,
    ExecuteToolRequest,
    DbQueryRequest,
...
from gyra_serve.building.app.api.schema_app import GptsAppQuery

from gyra_serve.building.config.service.service import Service as AppConfigService
...
@router.get("/v1/gyras/list", response_model=Result[List[GptsApp]])
async def get_gyras(user_code: str = None, sys_code: str = None):
    logger.info(f"get_gyras:{user_code},{sys_code}")
    try:
        ...
        logger.exception(f"get_gyras failed:{str(e)}")
...
async def query_tool(type: str, user_id: str = None):
    try:
        ...
        filter_tools = []
        for tool in tools:
            if tool.owner == user_id or tool.owner == "gyra":
...
async def get_agent_default_prompt(
    ...
    try:
        ...
        if agent is None:
            from gyra_serve.building.app.service.service import (
...
def _get_layered_identity_prompt(agent, language: str) -> str:
    ...
    """
    try:
        from gyra.agent.shared.prompt_assembly import get_registry, PromptRegistry
...
@router.get("/gyra/thinking/detail")
