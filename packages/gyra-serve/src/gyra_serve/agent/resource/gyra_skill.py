"""GyraSkill Resource - Manage AgentSkills from the database.
...
from gyra.agent.resource import PackResourceParameters, Resource, ResourceParameters
from gyra.agent.resource.agent_skills import AgentSkillResource
from gyra.util import ParameterDescription
    ...
    try:
        ...
        if _SYSTEM_APP:
            try:
                ...
                from gyra_serve.skill.api.schemas import SkillQueryFilter
...
class GyraSkillResourceParameters(PackResourceParameters):
    """Parameters for GyraSkill resource."""
...
class GyraSkillResource(AgentSkillResource):
    """GyraSkill resource class.
    ...
    def __init__(self, name: str = "GyraSkill Resource", **kwargs):
        """Initialize the GyraSkill resource."""
    ...
    def type(cls) -> str:
        ...
        Returns 'skill(gyra)' which is the same type as AgentSkillResource.
        """
        return "skill(gyra)"
    ...
    @classmethod
    def resource_parameters_class(cls, **kwargs) -> Type[GyraSkillResourceParameters]:
        ...
        @dataclasses.dataclass
        class _DynGyraSkillResourceParameters(GyraSkillResourceParameters):
        ...
        return _DynGyraSkillResourceParameters
    ...
    async def async_execute(
        ...
        **kwargs,
        ...
        For GyraSkill, this returns the skill prompt and metadata.
        """
...
def register_gyra_skill_resource(system_app):
    """Register the GyraSkill resource with the resource manager.

    This function should be called during system initialization to ensure
    the GyraSkill resource is available for Agents to bind and use.
    ...
    rm.register_resource(
        resource_cls=GyraSkillResource,
        resource_type=ResourceType.Tool,
        resource_type_alias="skill(gyra)",
        ignore_duplicate=True,
    )

    logger.info("GyraSkill resource registered successfully")
