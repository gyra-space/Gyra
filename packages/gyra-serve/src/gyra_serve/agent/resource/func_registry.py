...
from gyra.agent.resource.tool.base import ToolFunc, FunctionTool
from gyra_serve.agent.db.gpts_tool import GptsToolDao, GptsTool

logger = logging.getLogger(__name__)
gpts_tool_dao = GptsToolDao()
GYRA_TOOL_IDENTIFIER = 'gyra_tool'
...
def gyra_tool(
    ...
    def decorator(func: ToolFunc):
        ...
        func._to_register = {
            ...
            'owner': owner if owner is not None else 'gyra',
        ...
        setattr(wrapper, GYRA_TOOL_IDENTIFIER, True)
        return wrapper
...
class CentralFunctionRegistry:
    ...
    def register_standalone_function(self, func):
        """
        Registers standalone functions (not inside a class) that are decorated with @gyra_tool.
        """
        if asyncio.iscoroutinefunction(func):
            registration_info = func._to_register
    ...
    def scan_register_and_save(self, system_app: SystemApp):
        ...
        TOOL_MODULES_PACKAGE = 'gyra_ext.agent.agents'
                ...
                for name in dir(module):
                    ...
                    if inspect.isclass(obj):
                        ...
                        for method_name, method in inspect.getmembers(obj, inspect.isfunction):
                            if getattr(method, "__gyra_tool__", False):
                                has_gyra_tool = True
                                break
                        if has_gyra_tool:
                            self.register(obj)
                    elif inspect.isfunction(obj) and getattr(obj, "__gyra_tool__", False):
                        self.register(obj)
        ...
        try:
            from gyra.agent.resource.agent_skills import register_agent_skill_resource
