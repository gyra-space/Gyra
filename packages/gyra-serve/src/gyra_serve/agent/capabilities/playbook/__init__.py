...
实际实现在 ``gyra_serve.playbook.resource.playbook_capability``
...
本任务不动它;Task 4 的 capability 实现统一在 ``gyra_serve.playbook.resource``
模块,本 __init__ 仅做注册桥接。
"""

# 实现住在 playbook 模块(playbook_resource.py + playbook_capability.py)。
from gyra_serve.playbook.resource.playbook_capability import (  # noqa: F401
    PlaybookCapability,
    playbook_factory,
)
...
def register_capability_to(registry) -> None:
    ...
    被 ``CapabilityFactoryRegistry.discover()`` 扫 gyra_serve.agent.capabilities
    子包时调用。
    """
