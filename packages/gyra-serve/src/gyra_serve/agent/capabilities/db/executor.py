...
spec_service/DbSpecService 在 gyra-serve,本类用延迟导入,gyra-core 侧可定义。
"""
...
class DBExecutor(Executor):
    ...
    async def _fetch_db_prompt(self, requirement: DataRequirement) -> str:
        ...
        if spec_service is None or ds_id is None:
            # 无 spec_service(gyra-serve 不可用或无 spec),回退 connector 实时表名(异步)
    ...
    def _get_spec_service(self):
        """延迟加载 DbSpecService(gyra-serve)。core 测试环境不可用时返 None。"""
        try:
