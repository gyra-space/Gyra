...
def create_dattach_content(
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import create_dattach_content

        content = create_dattach_content(
            file_name="report.md",
            file_url="gyra-fs://bucket/path/report.md",
...
def create_dattach_list_content(
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import (
            create_dattach_list_content,
        )

        content = create_dattach_list_content(
            files=[
                {"file_name": "report.md", "file_url": "gyra-fs://bucket/report.md"},
                {"file_name": "data.csv", "file_url": "gyra-fs://bucket/data.csv"},
            ],
...
def render_dattach(
    ...
    **kwargs,
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import render_dattach

        # Use in Agent output
        output = f"\\n\\nAnalysis complete, generated report file:\\n{render_dattach('report.md', 'gyra-fs://bucket/report.md')}"
        ```
    """
    content = create_dattach_content(
        file_name=file_name,
...
def render_dattach_list(
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import render_dattach_list
        ...
        output = f"\\n\\nTask complete, generated files:\\n{
            render_dattach_list(
                [
                    {
                        'file_name': 'report.md',
                        'file_url': 'gyra-fs://bucket/report.md',
                    },
                    {
                        'file_name': 'data.csv',
                        'file_url': 'gyra-fs://bucket/data.csv',
                    },
                ]
            )
        }"
...
def attach(
    ...
    **kwargs,
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import attach
...
def attach_list(
    ...
    **kwargs,
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import attach_list
...
class DAttachBuilder:
    ...
    Example:
        ```python
        from gyra_serve.agent.file_io.dattach_utils import DAttachBuilder
        ...
        dattach_list = (
            DAttachBuilder()
            .add_file("report.md", "gyra-fs://bucket/report.md", 1024)
            .add_file("data.csv", "gyra-fs://bucket/data.csv", 2048)
            .title("Deliverables")
            .build_list()
        )
        ```
