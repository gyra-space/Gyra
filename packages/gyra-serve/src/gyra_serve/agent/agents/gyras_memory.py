...
- MetaGyrasPlansMemory: 计划存储
- MetaGyrasMessageMemory: 消息存储
- MetaAgentSystemMessageMemory: 系统消息存储
- MetaGyrasWorkLogStorage: 工作日志存储
- MetaGyrasKanbanStorage: 看板存储
- MetaGyrasTodoStorage: 任务列表存储
- MetaGyrasFileMetadataStorage: 文件元数据存储
"""
...
from gyra.agent.core.memory.gpts.base import AgentSystemMessageMemory
...
class MetaGyrasPlansMemory(GptsPlansMemory):
    def __init__(self):
...
class MetaGyrasWorkLogStorage:
    """基于数据库的 WorkLog 存储实现（用于 GptsMemory 集成）.

    类似于 MetaGyrasMessageMemory，封装 GptsWorkLogDao 以实现数据库持久化。
    """
...
class MetaGyrasKanbanStorage:
    """基于数据库的 Kanban 存储实现（用于 GptsMemory 集成）.

    类似于 MetaGyrasPlansMemory，封装 GptsKanbanDao 以实现数据库持久化。
    """
...
class MetaGyrasFileMetadataStorage:
    """基于数据库的文件元数据存储实现（用于 GptsMemory 集成）.

    与 MetaGyrasWorkLogStorage/MetaGyrasKanbanStorage/MetaGyrasTodoStorage 保持一致的架构，
