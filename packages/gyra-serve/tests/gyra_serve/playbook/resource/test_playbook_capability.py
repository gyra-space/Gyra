...
- discover() 扫 gyra_serve.agent.capabilities.playbook,注册 factory 到 _factories["playbook"]。
...
def test_playbook_factory_registered():
    """discover() 扫到 gyra_serve.agent.capabilities.playbook,
...
def test_playbook_factory_builds_capability_from_value_dict():
    ...
    from gyra_serve.playbook.resource.playbook_capability import (
        PlaybookCapability,
        playbook_factory,
    )
...
def test_build_pack_consumes_playbook_agent_resource():
    ...
    with patch(
        "gyra.agent.resource.manage.get_resource_manager",
