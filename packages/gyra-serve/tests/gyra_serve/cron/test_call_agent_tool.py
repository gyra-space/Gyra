...
from gyra.agent.tools.builtin.schedule.call_agent import CallAgentTool
from gyra.agent.tools.context import ToolContext


def test_call_agent_success_isolated_creates_new_conv():
    tool = CallAgentTool()
    with patch("gyra_serve.agent.agents.controller.multi_agents") as mock_ma:
...
def test_call_agent_shared_persists_session_id_under_cron_job():
    ...
    with patch("gyra_serve.agent.agents.controller.multi_agents") as mock_ma:
        mock_ma.app_chat_v3 = AsyncMock(return_value=(None, "agent-conv-1"))
        with patch(
            "gyra_serve.cron.service.service.session_id_by_conv_id",
            return_value="sess-1",
        ):
            with patch("gyra._private.config.Config") as mock_cfg:
