"""GptsTodoDao + MetaGyrasTodoStorage 持久化 round-trip 测试.
...
from gyra_serve.agent.db.gpts_todos_db import GptsTodoDao
...
async def test_storage_round_trip(db_session):
    """MetaGyrasTodoStorage 端到端：write -> read 拿回.

    修复前 todos 被丢弃，read 恒为 []。
    """
    from gyra.agent.core.memory.gpts.file_base import TodoItem, TodoStatus
    from gyra_serve.agent.agents.gyras_memory import MetaGyrasTodoStorage

    storage = MetaGyrasTodoStorage()
    todos = [
...
async def test_storage_does_not_collide_with_kanban(db_session):
    ...
    from gyra.agent.core.memory.gpts.file_base import TodoItem, TodoStatus
    from gyra_serve.agent.agents.gyras_memory import MetaGyrasTodoStorage
    from gyra_serve.agent.db.gpts_kanban_db import GptsKanbanDao
