"""Prometheus metrics exporter for Gyra.
...
via model_scan("gyra_ext.trace", SpanStorage).
"""
...
from gyra.util.tracer.base import Span, SpanStorage, SpanType, SpanTypeRunName
...
if PROMETHEUS_AVAILABLE:
    HTTP_REQUESTS_TOTAL = Counter(
        "gyra_http_requests_total",
        "Total number of HTTP requests",
        ["method", "path", "status"],
    )
    HTTP_REQUEST_DURATION = Histogram(
        "gyra_http_request_duration_seconds",
    ...
    TOOL_CALLS_TOTAL = Counter(
        "gyra_tool_calls_total",
        "Total number of tool calls",
        ["tool_name", "status"],
    )

    ACTIVE_CONVERSATIONS = Gauge(
        "gyra_active_conversations",
        "Number of currently active conversations",
    )

    SYSTEM_INFO = Info(
        "gyra",
        "Gyra system information",
    )
...
class PrometheusSpanStorage(SpanStorage):
    ...
    Automatically discovered by model_scan("gyra_ext.trace", SpanStorage)
    ...
    def _is_metrics_enabled() -> bool:
        ...
        return os.getenv("GYRA_ENABLE_PROMETHEUS", "true").lower() in (
            "true",
            "1",
            "yes",
        )
