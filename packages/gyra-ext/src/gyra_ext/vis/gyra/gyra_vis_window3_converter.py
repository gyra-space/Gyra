...
from gyra.agent import ActionOutput, ConversableAgent, BlankAction
from gyra.agent.core.action.report_action import ReportAction
from gyra.agent.core.file_system.file_tree import TreeManager, TreeNodeData
from gyra.agent.core.memory.gpts import GptsMessage, GptsPlan
from gyra.agent.core.memory.gpts.gpts_memory import AgentTaskContent, AgentTaskType
from gyra.agent.core.plan.planning_action import PlanningAction

from gyra.agent.core.reasoning.reasoning_action import (
    AgentAction,
    KnowledgeRetrieveAction,
)
from gyra.agent.core.schema import Status
from gyra.agent.core.user_proxy_agent import HUMAN_ROLE
from gyra.agent.expand.actions.agent_action import AgentStart
from gyra.agent.expand.actions.code_action import CodeAction
from gyra.agent.expand.actions.tool_action import ToolAction
from gyra.vis.schema import VisAttachListContent, VisAttachContent
from gyra.vis.vis_converter import SystemVisTag
from gyra_ext.vis.common.tags.gyra_attach import GyraAttach
from gyra_ext.vis.common.tags.gyra_plan import AgentPlan, AgentPlanItem
from gyra_ext.vis.common.tags.gyra_todo_list import TodoList
from gyra_ext.vis.common.tags.gyra_thinking import (
    GyraThinking,
    DrskThinkingContent,
)
from gyra_ext.vis.common.tags.gyra_tool import ToolSpace
from gyra_ext.vis.common.tags.gyra_work_space import (
    WorkSpaceContent,
    WorkSpace,
    FolderNode,
)
from gyra_ext.vis.common.tags.gyra_system_events import (
    SystemEvents,
    SystemEventsContent,
)
from .gyra_vis_incr_converter import GyraVisIncrConverter
from gyra_ext.vis.gyra.gyra_vis_converter import DrskVisTagPackage
from gyra_ext.vis.gyra.tags.gyra_agent_folder import AgentFolder
from gyra_ext.vis.gyra.tags.gyra_space_llm import LLMSpaceContent, LLMSpace
from gyra_ext.vis.gyra.tags.drsk_content import DrskTextContent, DrskContent

from gyra_ext.vis.vis_protocol_data import UpdateType
from ..common.tags.gyra_attach_list import GyraAttachList
from ...agent.actions.gyra_tool_action import GyraToolAction
...
ACTION_TASK_MAP = {
    ...
    GyraToolAction.name: "tool",
    # 有展示分类需求的再这里进行分类处理
}


class GyraIncrVisWindow3Converter(GyraVisIncrConverter):
    ...
    def __init__(self, paths: Optional[str] = None, **kwargs):
        ...
        # self._drsk_web_url = Config().GYRA_WEB_URL
        self._drsk_web_url = ""
        # Eviction tracking
    ...
    def system_vis_tag_map(self):
        return {
            ...
            SystemVisTag.VisThinking.value: DrskVisTagPackage.GyraThinking.value,
            ...
            SystemVisTag.VisAttach.value: GyraAttach.vis_tag(),
    ...
    async def gen_work_item(
        ...
        if thinking:
            ...
            vis_thinking = GyraThinking().sync_display(
        ...
        if llm_content_md:
            ...
            llm_vis_md = LLMSpace().sync_display(
                content=LLMSpaceContent(
                    ...
                    link_url=f"{self._drsk_web_url}/api/gyra/thinking/detail?message_id={message_id}",
                ).to_dict()
            )
    ...
    async def _render_terminate_files(
        ...
        # 渲染d-attach-list组件
        return self.vis_inst(GyraAttachList.vis_tag()).sync_display(
    ...
    def _create_placeholder_planning_space(self, conv_id: str) -> str:
        ...
        """
        from gyra_ext.vis.common.tags.gyra_planning_space import (
            PlanningSpace,
