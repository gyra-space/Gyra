...
(see gyra_ext.knowledge.tools) operate on the VaultFS the resource
exposes.
...
from gyra.agent.resource.base import Resource, ResourceParameters, ResourceType

if TYPE_CHECKING:
    from gyra_ext.knowledge.vaultfs import VaultFS
...
# Registry hook: set by gyra_app at startup so the resource can resolve
    ...
    async def get_vault(self) -> "VaultFS":
        ...
        if factory is None:
            raise RuntimeError(
                ...
                "gyra_ext.knowledge.resource.set_vault_factory(...) at "
