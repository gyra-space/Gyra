...
from gyra.agent import ConversableAgent, AgentContext, Resource
from gyra.agent.core.action.base import ToolCall
from gyra.agent.core.sandbox.tools.browser_tool import BROWSER_TOOLS
from gyra.agent.expand.actions.sandbox_action import SandboxAction
...
from gyra_ext.vis.gyra.tags.drsk_browser import DrskBrowser
...
class BrowserAction(SandboxAction):
    ...
    async def gen_view(
        ...
        try:
            ...
            if tool_result.success:
                ...
                item = {
                    "url": tool_result.url,
                    "description": "Gyra正在使用浏览器-" + tool_result.title,
                    "web_image": image_url,
    ...
    async def gen_content(self, tool_result: Any) -> Any:
        from gyra.agent.core.sandbox.tools.browser_tool import BrowserResult
