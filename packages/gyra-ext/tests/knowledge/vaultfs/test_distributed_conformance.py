...
- `KNOWLEDGE_RELATIONAL_DSN`: SQLAlchemy async DSN for relational metadata
  (e.g. `postgresql+asyncpg://user:pass@localhost/gyra_test` or
  `mysql+asyncmy://user:pass@localhost/gyra_test`)
...
async def vault():
    from gyra_ext.knowledge.vaultfs import DistributedVaultFS
