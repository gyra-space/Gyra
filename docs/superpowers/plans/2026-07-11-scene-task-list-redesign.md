...
**Tech Stack:** Python 3 / asyncio / SQLAlchemy(后端 gyra-serve),React + Next.js + Ant Design + ahooks + TypeScript(前端 web/),pytest(asyncio_mode=auto)。
...
**后端(2 文件)**:
- `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py` —— 扩 `create_task_from_tool`:签名加 `model_name`;创建后 detached 跑 `start`+`run_task`;新增 `_summarize_task_title` helper + detached 标题协程。
- `packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py` —— `aggregation_chat` 的 `playbook_command` 分支:从 `chat_in_params` 抽 `model` 透传;`_user_text` 为空时拒绝创建。
...
**测试**:
- `packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py` —— 新建,`create_task_from_tool` 单元测试。
- 扩 `packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py` —— 加 `_extract_model` 测试。
...
**Files:**
- Modify: `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py`
- Test: `packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py` (Create)

**Interfaces:**
- Consumes: `task_service.start(task_id) -> TaskResponse`(`task/service/service.py:138`);`task_service.update(TaskRequest) -> TaskResponse`(`task/service/service.py:83`);`task_service.get_by_id(task_id) -> Optional[TaskResponse]`(`task/service/service.py:99`);`playbook_runtime.run_task(system_app, task_id, user_code=None, sys_code=None) -> Dict`(`playbook/runtime.py:34`,async);`AIWrapper` + `ModelConfigCache`(`gyra.agent.util.llm.*`)。
...
创建 `packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py`:
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py -v`
Expected: FAIL —— `_task_creator` 无 `asyncio` / `playbook_runtime` 模块属性,或 `_summarize_task_title` 未定义。

- [ ] **Step 3: 写最小实现**

整体替换 `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py`:
...
def create_task_from_tool(
    ...
    from gyra_serve.playbook import runtime as playbook_runtime  # noqa: F401  供测试 monkeypatch
...
```bash
git add packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py
...
**Files:**
- Modify: `packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py:991-1140`
- Test: `packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py`
...
追加到 `packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py` 末尾:
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py::test_extract_model_returns_model_name -v`
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py -v`
...
Run: `cd /Users/tuyang/GitHub/Gyra/web && cat package.json | grep -E '"(test|jest|vitest)"' && npx tsc --noEmit -p tsconfig.json 2>&1 | head`
...
Run: `cd /Users/tuyang/GitHub/Gyra/web && npx tsc --noEmit -p tsconfig.json`
...
shell 是组合组件、依赖 ahooks/gyra-app API,纯单测成本高。本任务的逻辑核心是"有活跃任务时起轮询、无则停",抽成一个纯函数测试。
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_task_creator.py packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py -v`
Expected: PASS(全部)。
...
Run: `cd /Users/tuyang/GitHub/Gyra && git diff --stat main...HEAD -- packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py web/src/app/workspaces/detail/scene-task-rail.tsx web/src/app/workspaces/detail/scene-workspace-shell.tsx web/src/app/workspaces/detail/scene-workspace.css web/src/app/workspaces/detail/agent-workspace-input.tsx`
