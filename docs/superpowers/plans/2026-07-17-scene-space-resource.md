...
**Tech Stack:** Python 3 / asyncio / SQLAlchemy(gyra-serve),RFC-005 资源协议(ResourceProtocol/Contribution/CapabilityPack),pytest。
...
**新建:**
- `packages/gyra-serve/src/gyra_serve/workspace/scene_resource.py` — `WorkspaceSceneResource`(ResourceProtocol)+ `WorkspaceSceneConfig`;`build_scene_management_tools`(四类管理工具全集,复用 read/write tools + 补齐)。
- `packages/gyra-serve/src/gyra_serve/workspace/scene_resource_assembler.py` — `SceneResourceAssembler.assemble(scene_resource_assembler.py)` lobby/workbench 装配产出 `List[AgentResource]`。
- `packages/gyra-serve/src/gyra_serve/workspace/scene_capability.py` — `workspace_scene` capability factory(注册到 `CapabilityFactoryRegistry`),从 AgentResource.value 还原 `WorkspaceSceneResource` Capability。
- `packages/gyra-serve/src/gyra_serve/playbook/resource/playbook_capability.py` — `playbook` capability factory,从 AgentResource.value 还原 PlaybookResource Capability。
- 测试: `tests/gyra_serve/workspace/test_scene_resource.py`、`test_scene_resource_assembler.py`、`test_scene_capability.py`;`tests/gyra_serve/playbook/resource/test_playbook_capability.py`。

**修改:**
- `packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py` — `_inject_workspace_context` 移除 204-241;删 `build_workspace_toolkit` import(88);保留 171-202。
- `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/toolkit.py` — 删 `build_workspace_toolkit`/`WorkspaceControlAgent`(无其他调用者)。保留 LAYER 常量(若 read_tools 用到)。
- `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/__init__.py` — 删 `build_workspace_toolkit` 导出。
- `packages/gyra-app/src/gyra_app/openapi/api_v1/api_v1.py` — `chat_completions` 端点预处理层调装配器。
- `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/write_tools.py` — 补齐 `create_playbook`/`update_playbook`/`delete_playbook`/`resolve_intervention`/`abort_intervention` 工具(供 scene_resource 复用)。
- `packages/gyra-core/src/gyra/agent/capabilities/registry.py` 或对应 `__init__.py` — 注册 `workspace_scene`/`playbook` factory(按现有 discover 机制)。
- `packages/gyra-serve/src/gyra_serve/playbook/service/service.py` — 确认 `create_playbook`/`update_playbook`/`delete_playbook` service 方法存在(写工具依赖);缺则补。
...
- Test: `packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_write_tools.py` (Create if absent)
...
grep -nE "def create\b|def update\b|def delete\b|def resolve_and_execute|def abort" \
  packages/gyra-serve/src/gyra_serve/playbook/service/service.py \
  packages/gyra-serve/src/gyra_serve/intervention/service/service.py
```
...
def test_create_playbook_tool_calls_playbook_service_create():
    ...
    from gyra_serve.workspace.agent_tools.write_tools import build_scene_write_tools
    with patch(
        "gyra_serve.workspace.agent_tools.write_tools.get_playbook_service"
    ) as mks:
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_write_tools.py -v`
...
def build_scene_write_tools(
    ...
    from gyra_serve.workspace.agent_tools.read_tools import get_playbook_service
...
git add packages/gyra-serve/src/gyra_serve/workspace/agent_tools/write_tools.py \
  packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/test_write_tools.py
...
**Files:**
- Create: `packages/gyra-serve/src/gyra_serve/workspace/scene_resource.py`
- Test: `packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource.py`

**Interfaces:**
- Consumes: Task 1 的 `build_scene_write_tools`;`build_read_tools`(read_tools.py);`ResourceProtocol/Contribution/Slot/Lifetime/CacheScope`(`gyra.core.interface.resource.{protocol,bundle}`);`FunctionTool`(`gyra.agent.resource.tool.base`)。
...
创建 `packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource.py`(若 `workspace/` 测试目录无 `__init__.py` 则建):
...
def test_declare_produces_system_and_tools_contributions():
    ...
    with patch("gyra_serve.workspace.scene_resource.build_scene_management_tools") as mtools:
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource.py -v`
Expected: FAIL — 模块不存在。
...
def build_scene_management_tools(workspace_id: int, conv_uid: str) -> List[FunctionTool]:
    ...
    from gyra.component import CFG
...
> 注:`Slot.SYSTEM` 的枚举访问方式(Slot.SYSTEM vs Slot["SYSTEM"])以 `gyra.core.interface.resource.bundle` 实际定义为准;若 Slot 是 Enum,用 `Slot.SYSTEM`;若 str-like,直接字符串。实现时按 PlaybookResource 里的用法对齐(playbook_resource.py:253 用 `slot=Slot.SYSTEM`,照抄)。
...
```bash
git add packages/gyra-serve/src/gyra_serve/workspace/scene_resource.py \
  packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource.py
...
**Files:**
- Create: `packages/gyra-serve/src/gyra_serve/workspace/scene_capability.py`
- Modify: `packages/gyra-core/src/gyra/agent/capabilities/registry.py`(或 `__init__.py`,按 discover 机制注册)— 让 `workspace_scene` type_key 的 factory 被发现
- Test: `packages/gyra-serve/tests/gyra_serve/workspace/test_scene_capability.py`
...
```bash
cd /Users/tuyang/GitHub/Gyra
sed -n '1,80p' packages/gyra-core/src/gyra/agent/capabilities/registry.py
ls packages/gyra-core/src/gyra/agent/capabilities/*/  # 看现有 capability 目录结构
```
...
def test_workspace_scene_factory_builds_capability_from_value():
    from gyra_serve.workspace.scene_capability import workspace_scene_factory
    import json
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/test_scene_capability.py -v`
...
from gyra.core.interface.resource.bundle import Contribution
from gyra_serve.workspace.scene_resource import (
    WorkspaceSceneConfig, WorkspaceSceneResource,
)
...
class WorkspaceSceneCapability:
    """Capability 包装 WorkspaceSceneResource.declare。基类/接口按 gyra.agent.capabilities 现有 Capability。"""
...
注册:按 Step 1 记录的 discover 约定,在 `capabilities/registry.py` 的 import 列表/`__init__.py` 加 `from gyra_serve.workspace.scene_capability import register as register_scene_cap`,并在 discover 扫描时调用 `register_scene_cap(registry_factory)`。具体落点以 Step 1 现有约定为准(若 gyra-serve 的 capability 通过独立 register 函数被 gyra-core discover,则在 discover 路径加入;若需手动 register,在 app 启动初始化处加)。

- [ ] **Step 5: 运行确认通过**

Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra_serve/tests/gyra_serve/workspace/test_scene_capability.py -v`
Expected: PASS。

- [ ] **Step 6: 提交**

```bash
git add packages/gyra-serve/src/gyra_serve/workspace/scene_capability.py \
  packages/gyra-core/src/gyra/agent/capabilities/registry.py \
  packages/gyra-serve/tests/gyra_serve/workspace/test_scene_capability.py
...
**Files:**
- Create: `packages/gyra-serve/src/gyra_serve/playbook/resource/playbook_capability.py`
- Modify: `packages/gyra-serve/src/gyra_serve/playbook/resource/playbook_resource.py`(加 `to_agent_resource` 静态方法)
- Test: `packages/gyra-serve/tests/gyra_serve/playbook/resource/test_playbook_capability.py`
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/playbook/resource/test_playbook_capability.py -v`
Expected: FAIL。
...
```bash
git add packages/gyra-serve/src/gyra_serve/playbook/resource/playbook_capability.py \
  packages/gyra-serve/src/gyra_serve/playbook/resource/playbook_resource.py \
  packages/gyra-serve/tests/gyra_serve/playbook/resource/test_playbook_capability.py \
  packages/gyra-core/src/gyra/agent/capabilities/registry.py
...
**Files:**
- Create: `packages/gyra-serve/src/gyra_serve/workspace/scene_resource_assembler.py`
- Test: `packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource_assembler.py`
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource_assembler.py -v`
Expected: FAIL。
...
```bash
git add packages/gyra-serve/src/gyra_serve/workspace/scene_resource_assembler.py \
  packages/gyra-serve/src/gyra_serve/workspace/scene_resource.py \
  packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource_assembler.py
...
**Files:**
- Modify: `packages/gyra-app/src/gyra_app/openapi/api_v1/api_v1.py`(394 端点内,446 后)
- Test: `packages/gyra-app/tests/` 下端点测试(若有)或单测装配器接入函数
...
测试(放 `packages/gyra-app/tests/test_scene_resource_endpoint.py`,若无测试框架则按现有 api_v1 测试约定):

```python
from unittest.mock import patch, MagicMock
from gyra_app.openapi.api_v1.api_v1 import _assemble_scene_resources
...
def test_assemble_scene_resources_calls_assembler_with_workspace():
    with patch("gyra_serve.workspace.scene_resource_assembler.SceneResourceAssembler.assemble") as m:
...
Run(若有 gyra-app 测试运行器): `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-app/tests/test_scene_resource_endpoint.py -v 2>/dev/null || python -c "from gyra_app.openapi.api_v1.api_v1 import _assemble_scene_resources; print('import ok')"`
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-app/tests/test_scene_resource_endpoint.py -v 2>/dev/null || python -c "import gyra_app.openapi.api_v1.api_v1 as m; m._assemble_scene_resources({}, 'c1'); print('ok')"`
Expected: 测试 PASS 或 `ok`。

- [ ] **Step 5: 提交**

```bash
git add packages/gyra-app/src/gyra_app/openapi/api_v1/api_v1.py \
  packages/gyra-app/tests/test_scene_resource_endpoint.py
...
**Files:**
- Modify: `packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py`(移除 `_inject_workspace_context` 的 204-241 段 + 88 import)
- Modify: `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/toolkit.py`(删 `build_workspace_toolkit`/`WorkspaceControlAgent`)
- Modify: `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/__init__.py`(删导出)
...
追加到 `packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py`:
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py::test_inject_workspace_context_no_longer_appends_extra_agents -v`
...
删除 `agent_chat.py:88` `from gyra_serve.workspace.agent_tools.toolkit import build_workspace_toolkit`(若仅此处用)。
...
```bash
cd /Users/tuyang/GitHub/Gyra
python -m pytest packages/gyra-serve/tests/gyra_serve/workspace/agent_tools/ packages/gyra-serve/tests/gyra_serve/agent/agents/chat/ packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource.py packages/gyra-serve/tests/gyra_serve/workspace/test_scene_resource_assembler.py packages/gyra-serve/tests/gyra_serve/workspace/test_scene_capability.py packages/gyra-serve/tests/gyra_serve/playbook/resource/test_playbook_capability.py -v
```
...
git add packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py \
  ...
  packages/gyra-serve/tests/gyra_serve/agent/agents/chat/test_agent_chat_playbook_command.py
...
Run: `cd /Users/tuyang/GitHub/Gyra && python -m pytest packages/gyra-serve/tests/ packages/gyra-app/tests/ -v 2>&1 | tail -20`
Expected: 本计划相关测试全 PASS;预先存在的无关失败记录但不归责本计划。

- [ ] **Step 2: 端到端冒烟(用户/人工)**

起 gyra 后端 + web 前端,在场景空间:
...
Run: `cd /Users/tuyang/GitHub/Gyra && git diff --stat 7ea180ff..HEAD`
...
- `build_pack` 经 factory 注册消费(Task 3/4 的 `register`);若 gyra-serve capability 不被 gyra-core discover 自动扫,需在 app 启动初始化手动 register(Task 3 Step 4 已注)。
