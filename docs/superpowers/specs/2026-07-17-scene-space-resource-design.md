...
### 1.1 崩溃现象
场景空间 Agent 对话发起即崩,日志(`logs/gyra_default.log`):
```
...
- **位置:** `packages/gyra-serve/src/gyra_serve/workspace/scene_resource_assembler.py`(`workspace/` 域,不进 `agent/` 包)。
...
- **位置:** `packages/gyra-serve/src/gyra_serve/workspace/scene_resource.py`(类比 `playbook/resource/playbook_resource.py`)。
...
- **位置:** `packages/gyra-app/src/gyra_app/openapi/api_v1/api_v1.py`,`chat_completions`(394)内,446 后(`dialogue.ext_info` 准备好后)、调 `multi_agents.app_chat_v2/v3`(546/565/585)前。
