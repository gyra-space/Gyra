...
**后端 Task 模型** `packages/gyra-serve/src/gyra_serve/task/`:
...
**改动文件**:`packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py`、`packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py`(playbook_command 分支)。
...
新增 helper:`async def _summarize_task_title(user_text: str, playbook_name: str | None, model: str | None) -> str`。按 `packages/gyra-serve/src/gyra_serve/knowledge/ingest.py:517-589`(_call_llm)的已有模式,用 `AIWrapper` + `ModelConfigCache`(`gyra.agent.util.llm.llm_client` / `model_config_cache`)做单次 LLM 调用。不需 `system_app`(这两个是模块级单洞,启动时已注册模型)。
...
**后端(2 文件)**:
- `packages/gyra-serve/src/gyra_serve/workspace/agent_tools/_task_creator.py` — `create_task_from_tool` 创建后 detached `start`+`run_task`;新增 `_summarize_task_title` helper + detached 标题协程;签名加 `model_name`。
- `packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py` — `playbook_command` 分支透传 `model_name`;`_user_text` 为空时拒绝创建。
