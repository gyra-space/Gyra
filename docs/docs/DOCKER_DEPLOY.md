...
| `GYRA_LANG` | `en` | 否 | 界面语言：`en` 或 `zh` |
| `WEB_SERVER_PORT` | `7777` | 否 | 后端端口映射 |
| `DB_TYPE` | `sqlite` | 否 | 数据库类型：`sqlite` 或 `mysql` |
| `GYRA_CONFIG_FILE` | `configs/gyra-docker.toml` | 否 | 配置文件路径 |
...
后端通过 TOML 配置文件控制核心行为，位于 `configs/` 目录。Docker 部署默认使用 `configs/gyra-docker.toml`。

可用配置文件：

| 配置文件 | 说明 |
|----------|------|
| `gyra-docker.toml` | Docker 部署专用（推荐） |
| `gyra-proxy-openai.toml` | OpenAI 配置 |
| `gyra-proxy-deepseek.toml` | DeepSeek 配置 |
| `gyra-proxy-claude.toml` | Anthropic Claude 配置 |
| `gyra-proxy-gemini.toml` | Google Gemini 配置 |
| `gyra-proxy-kimi.toml` | Moonshot Kimi 配置 |
| `gyra-proxy-tongyi.toml` | 通义千问配置 |
| `gyra-proxy-ollama.toml` | Ollama 本地模型配置 |
| `gyra-siliconflow.toml` | SiliconFlow 配置 |

切换配置文件：

```bash
# 在 .env 中修改
GYRA_CONFIG_FILE=configs/gyra-proxy-deepseek.toml
```
...
GYRA_CONFIG_FILE=configs/gyra-proxy-claude.toml
```
...
| Volume 名称 | 容器挂载路径 | 说明 |
|-------------|-------------|------|
| `gyra-metadata` | `/app/pilot/meta_data` | SQLite 数据库、元数据 |
| `gyra-vectordata` | `/app/pilot/data` | ChromaDB 向量存储 |
| `gyra-messages` | `/app/pilot/message` | 对话消息数据 |
| `gyra-logs` | `/app/logs` | 应用日志 |
| `gyra-mysql-data` | `/var/lib/mysql` | MySQL 数据（仅 mysql profile） |
...
# 备份所有数据卷
docker run --rm \
  -v gyra-metadata:/data/metadata \
  -v gyra-vectordata:/data/vectordata \
  -v gyra-messages:/data/messages \
  -v $(pwd)/backup:/backup \
  alpine tar czf /backup/gyra-backup-$(date +%Y%m%d).tar.gz -C /data .
```
...
docker run --rm \
  ...
  -v gyra-messages:/data/messages \
  -v $(pwd)/backup:/backup \
  alpine tar xzf /backup/gyra-backup-20250224.tar.gz -C /data
...
services:
  frontend:
    labels:
      - "traefik.enable=true"
      - "traefik.http.routers.gyra.rule=Host(`gyra.yourdomain.com`)"
      - "traefik.http.routers.gyra.tls.certresolver=letsencrypt"
      - "traefik.http.services.gyra.loadbalancer.server.port=3000"
    ports: []  # 不直接暴露端口
    networks:
      - gyranet
      - traefik
...
docker compose exec backend python -c "import gyra; print(gyra.__file__)"
```
...
后端自身也内嵌了静态 Web 文件（如果存在 `packages/gyra-app/src/gyra_app/static/web/`）。
...
```env
GYRA_CONFIG_FILE=configs/my-custom-config.toml
```
...
LLM_MODEL_PROVIDER=proxy/openai
GYRA_CONFIG_FILE=configs/gyra-proxy-ollama.toml
```
...
├── configs/
│   ├── gyra-docker.toml      # Docker 专用配置
│   ├── gyra-proxy-openai.toml
│   ├── gyra-proxy-deepseek.toml
│   ├── gyra-proxy-claude.toml
│   └── ...
├── docker/
