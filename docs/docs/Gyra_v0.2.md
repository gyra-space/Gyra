...
Gyra V0.2版本, 构建面向未来的Multi-Agent开发与运行产品框架，主要包含以为核心能力:
## 1. 多智能体构建框架，主要功能包括
- 提示词编写
    - 系统提示词编写
    - 用户提示词编写
...
* AI-SRE(OpenRca根因定位)
  ...
  -  你可以通过链接, 或者下述命令进行下载：
    ...
    ```
  - 下载完成后, 将数据解压到 ${gyra项目}/pilot/datasets。
...
### 3. 多智能体构建
在Gyra V0.2版本, 一个重要的功能是提供了多智能体的构建与调试能力。打开左侧菜单栏的【应用管理】界面, 即可开始智能体的构建与管理。 
...
#### 界面布局
在Gyra的设计中，每一类智能体都会有特定的工作空间，可以通过界面布局来选择不同智能体的工作空间。Gyra内置了几种常见的工作空间的展示，在实际的使用中也可以自行拓展。  
...
## 小结
Gyra V0.2是围绕Multi-Agent构建的一套体系化的产品框架，支持复杂业务场景下多智能体协同处理，期望通过Gyra的开放性，促进AI原生应用开发领域的发展。 
