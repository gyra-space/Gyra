...
To run gyra with the local model. You can modify the `configs/gyra-local-qwen.toml` configuration file to specify the model path and other parameters.
...
To run gyra with the local model. You can modify the `configs/gyra-local-vllm.toml` configuration file to specify the model path and other parameters.
...
```bash
uv run gyra start webserver --config configs/gyra-local-vllm.toml
```
