...
```
┌─────────────────────────── 原始资产层 ───────────────────────────┐
│ DB: gyra_serve/datasource (已有, 不动)                          │
...
└──────────────┬──────────────────────────────────────────────────┘
               ...
               ▼
┌────────────────────────── 语义资产层 ────────────────────────────┐
│ 硬语义层 [新建 gyra_serve/ecp]      软知识层 [复用 knowledge]    │
...
## 2. 新建模块：gyra_serve/ecp

照 Usage serve 骨架（commit aa3fa198 模板）：

```
packages/gyra-serve/src/gyra_serve/ecp/
...
注册点：`gyra_app/initialization/serve_initialization.py` 扫描列表 + `register_serve_apps`；
表模型加入 `db_model_initialization.py` 的 `_MODELS`；MySQL 同步 `assets/schema/gyra.sql`。
...
1. **补 Excel extractor**：`gyra_ext/knowledge/extractors/builtin.py` 加 `ExcelExtractor`
...
| 问题 | 决策 | 理由 |
|---|---|---|
| 模块形态 | **独立 `gyra_serve/ecp` 模块，单向依赖 knowledge/datasource/cron** | 硬层与 vault 粒度/关切不同，内建会双向污染；软层重写是浪费；单向依赖保证两模块独立演进 |
