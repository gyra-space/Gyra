...
**测试文件**: `packages/gyra-core/tests/agent/test_history_compaction.py`
**总行数**: 1,487 行
**测试类**: 19 个
**测试用例**: 100+ 个
...
```bash
# 运行所有测试
python -m pytest packages/gyra-core/tests/agent/test_history_compaction.py -v

# 运行特定测试类
python -m pytest packages/gyra-core/tests/agent/test_history_compaction.py::TestPipelineLayer1Truncation -v

# 带覆盖率报告
python -m pytest packages/gyra-core/tests/agent/test_history_compaction.py --cov=gyra.agent.core.memory --cov-report=html
```
