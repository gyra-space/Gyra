...
L1 Loop 的核心是 **Thinking（思考）→ Acting（行动）→ Verify（验证）** 的循环。当前工程由 [run\_loop.py](../packages/gyra-core/src/gyra/agent/core/v2/run_loop.py) 承载多轮循环，调用 [runtime.py](../packages/gyra-core/src/gyra/agent/core/v2/runtime.py) 的 `run_step` 逐轮推进，直到 LLM 不再 emit `tool_call` / 达到 `max_steps` / 失败 / 挂起：
...
Loop 的挂起状态（`AWAITING_USER` / `AWAITING_TOOL_PERMISSION` / `AWAITING_SUB_AGENT`）会中断 turn，等待人介入或子智能体完成后再恢复。韧性执行细节（死循环检测、上下文压缩、输出截断、历史裁剪）收敛在 [react\_master\_agent](../packages/gyra-core/src/gyra/agent/expand/react_master_agent/react_master_agent.py) 的 ReAct 循环中：
...
LLM 会在卡住时反复调用同一个工具、同一组参数。[doom\_loop\_detector.py](../packages/gyra-core/src/gyra/agent/expand/react_master_agent/doom_loop_detector.py) 用 `IntelligentDoomLoopDetector` 对"工具名 + 规范化参数"做 SHA256 哈希，检测连续相同调用：
...
记忆只有"被用上"才有价值。[recall\_tracker.py](../packages/gyra-core/src/gyra/storage/memory/recall_tracker.py) 的 `RecallTracker` 通过可插拔的 `RecallStatsBackend` 持久化统计（serve 层默认走主库表 `gyra_memory_recall_stats`，见 [recall\_stats\_store.py](../packages/gyra-serve/src/gyra_serve/memory/recall_stats_store.py)；单机/测试可退化为 SQLite 文件或纯内存），记录每条记忆的：
...
L2 记忆不只是"下次任务加速"，还会进一步**单向沉淀为 workspace 资产**，把个体经验变成组织资产。这是当前工程飞轮中"资产环"的输入源。[sediment\_service.py](../packages/gyra-serve/src/gyra_serve/workspace_asset/service/sediment_service.py) 的 `SedimentPipeline` 编排：
...
* **单向边界**：L2 记忆晋升 = agent 个体经验升级为重要记忆（私有）；沉淀为 ws\_asset = 组织资产（共享，需人 review）。单向 L2 → 资产，不反向污染个人记忆。
...
L3 Loop 的载体是**场景空间**。Gyra 的设计立场（来自 [SCENARIO\_WORKSPACE\_PRODUCT\_DESIGN.md](./SCENARIO_WORKSPACE_PRODUCT_DESIGN.md)）：
...
* **强制 distill**：Task 关闭前强制校验沉淀完整性，未完成 distill 不允许 close（UI 不放行）。产出物沉淀为 Asset（historical\_artifact / case / runbook / metric / template 等）。
...
* **L1 内部**：[run\_loop.py](../packages/gyra-core/src/gyra/agent/core/v2/run_loop.py) 的 while 循环每轮 thinking → act → verify，直到 `max_steps` 或不再 emit 工具调用。

* **L3 闭环**：Trigger 持续监听 → fire 创建 Task → Playbook run\_task 驱动 Agent → 产出 Artifact → Delivery 投递 → distill 沉淀为资产 → 演化剧本，形成"空间越用越厚"的飞轮。
...
未来路线（[SCENARIO\_WORKSPACE\_DESIGN.md](./SCENARIO_WORKSPACE_DESIGN.md)）按 P0-P8 分期推进：从 IA 收敛、能力视图、收件箱收敛、剧本健康度，到 Host 类交付与托管运行时，逐步把"面向最终交付构建空间"落地。
