...
当前 Gyra 没有真正的权限控制：`get_user_from_headers()` 始终返回 mock admin 用户，所有 API 无认证无鉴权。项目已有 OAuth2 登录系统和 user_groups 数据模型，但未形成完整的权限闭环。需要设计一套**默认关闭、开启即生效**的可插拔 RBAC 权限架构。
...
```
packages/gyra-app/src/gyra_app/feature_plugins/permissions/
├── __init__.py
...
**文件**: `packages/gyra-serve/src/gyra_serve/utils/auth.py`
...
if _enabled("permissions"):
    from gyra_app.feature_plugins.permissions.api import router as permissions_router
...
在 `~/.gyra/gyra.json` 的 `feature_plugins` 中添加：
...
| 文件 | 修改类型 | 说明 |
|------|---------|------|
| `packages/gyra-serve/src/gyra_serve/utils/auth.py` | **修改** | 核心：条件认证 + UserRequest 扩展 |
| `packages/gyra-app/src/gyra_app/feature_plugins/catalog.py` | **修改** | 添加 permissions manifest |
| `packages/gyra-app/src/gyra_app/feature_plugins/bootstrap.py` | **修改** | 注册 permissions 路由 + 种子数据 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/__init__.py` | **新增** | 模块入口 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/models.py` | **新增** | 4 张表 ORM 实体 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/dao.py` | **新增** | 数据访问层 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/service.py` | **新增** | 权限聚合 + 缓存 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/checker.py` | **新增** | require_permission() 依赖 |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/api.py` | **新增** | 管理 API |
| `packages/gyra-app/src/gyra_app/feature_plugins/permissions/seed.py` | **新增** | 种子数据 |
