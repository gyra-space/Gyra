...
> **场景空间是组织/团队的工作单元，不是个人的。** Gyra 不把"个人空间"作为产品主轴。
...
位置：`packages/gyra-serve/src/gyra_serve/workspace/inbox/`（新建子包，含 models / service / api）

```
gyra_serve_workspace_inbox_item
├── id              Integer PK
...
`gyra_serve/task/models/models.py` TaskEntity 加：
...
`gyra_serve/intervention/models/models.py` InterventionEntity 加：
