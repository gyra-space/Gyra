...
本文档是 Gyra 从"Agent 配置平台"演进为"场景化 AI 团队空间"的总体设计与落地蓝图。目标读者：架构 / 后端 / 前端 / 产品。读完应能回答：
...
> 本章回答一个具体问题：一个普通用户（不是 Builder、不是管理员）登录 Gyra 后，看到的世界是什么样的。

### 1.1 核心立场

**场景空间是组织/团队的工作单元，不是个人的。** 但用户作为"参与者"会同时存在于多个空间中，每个用户还有一个轻量的"个人视图"作为跨空间的入口。

回顾第 0 章的命题：本地工具（Claude Code / Hermes）在"个人单机"场景上对云端服务是降维打击。所以 Gyra **不把"个人空间"作为产品主轴**，但保留一个**"我的视图"**（My View）作为跨空间的总入口——它是过滤器，不是独立的 Workspace 实体。
...
Gyra 当前的信息架构以 `gpts_app_config`（Agent 应用）为脊柱：用户登录后第一屏是 HomeChat，所有功能模块（application / knowledge-vault / models / database / mcp / agent-skills）都围绕"如何配置和使用一个 Agent"展开。已有能力包括：

- **Agent 体系**：`gyra-core/agent/` 提供多 Agent 协作、记忆、计划、看板；`gyra-serve/agent/` 提供 App 管理、版本、发布
- **Skill 系统**：可复用的能力封装（`gyra_serve.skill`）
- **MCP 集成**：`gyra_serve.mcp` 已支持外部 MCP server 注册与调用
- **DataResource**：`connect_config` / `db_spec` / `table_spec` / `db_learning_task` 数据源与数据学习任务
- **Scheduler**：`gyra_app/initialization/scheduler.py`（APScheduler）
- **Channels**：`gyra-core/agent/channels/` 已具备多渠道接入能力
...
> **Gyra 是面向场景的 AI 团队空间平台。每个空间是一个组织单元（按场景/任务域划分），人与 Agent 按剧本协作完成任务，沉淀的知识、口径、模板、案例让团队在同类场景下越来越高效。**
...
├─────────────────────────────────────────────────────────┤
│ API Layer (gyra_app/openapi/api_v2/)                  │
│ 新增: workspace_api / task_api / playbook_api /         │
│       artifact_api / asset_api / delivery_api /         │
│       intervention_api                                  │
├─────────────────────────────────────────────────────────┤
│ New Serve Layer (packages/gyra-serve/src/gyra_serve)│
...
| 空间层 ↔ 引擎层 | Task 执行通过现有 `gyra_serve.agent` 启动 AgentRun；空间层不直接调 Agent |
...
```sql
-- 全局 SkillBundle 注册表（在 gyra_serve.skill 下）
...
复用现有 `gyra-core/agent/channels/`：飞书 / 钉钉 / 邮件 / 站内信。介入请求通过 channels 推送，DeepLink 回到 Workspace 处理。
...
实现位置：`gyra_serve.workspace.context_builder`，作为 Agent 启动前的 prompt 注入器。
...
|---|---|---|---|
| **notify** | `email` / `feishu` / `dingtalk` / `webhook` / `in_app` | 把 Artifact 格式化后推送 | 现有 `gyra-core/agent/channels/` |
...
| **publish** | `file_store` | 写入对象存储 | 现有 `gyra_serve.file` |
| **publish** | `asset_library` | 沉淀到本空间 Asset 库 | `gyra_serve.asset` |
| **execute** | `action_executor` | **执行 operation_plan** | MCP / Skill / 现有工具体系 |
| **execute** | `downstream_playbook` | 触发另一个 Playbook 派生新 Task | `gyra_serve.task` |
...
**② 渠道复用现有能力**：notify 类复用 `channels/`；publish 类的 `data_table` 复用 `connect_config`，`file_store` 复用 `gyra_serve.file`；execute 类的 `action_executor` 复用现有 MCP / Skill / 工具体系；**host 类复用容器运行时（K8s/Docker）和现有 `gyra_serve.file` 存构建产物**——不重造执行/托管通道。
...
| 现有能力 | 在新架构中的角色 |
|---|---|
| `gyra-core/agent/` | Agent 执行引擎，被 AgentRun 调用 |
| `gyra_serve.agent` | Agent 注册、版本、发布；空间通过 `default_agent_app_code` 配置，不作为 WorkspaceResource 订阅 |
| `gyra_serve.skill` | Skill 注册 + **SkillBundle 管理**（场景能力核心载体）；作为 WorkspaceResource `type=skill_bundle` / `type=skill` 引用 |
| `gyra_serve.mcp` | MCP server 管理，作为 WorkspaceResource `type=mcp` 引用；execute 类交付的 action_executor 通过 MCP 执行操作 |
| `gyra_serve.knowledge` | knowledge-vault Space 作为 WorkspaceResource `type=knowledge_space` 引用 |
| `gyra_serve.conversation` | 对话管理，新增 `task_id` 关联 |
| `gyra_app/initialization/scheduler.py` | APScheduler 作为 TriggerSource 的 timer 实现 |
| `gyra-core/agent/channels/` | 介入通知 + Notify 类 Delivery 渠道 |
...
| `gyra_serve.file` | 文件存储，Publish 类 `file_store` 渠道复用 |
| `db_learning_task` / `db_learning_subtask` | Task + Subtask 模型的参考模板 |

### 11.2 新增（在 gyra-serve 层）

```
gyra_serve.workspace         # Workspace CRUD + 成员
gyra_serve.workspace_resource # 统一资源管理（3 类资源，引用现有物理资源）
gyra_serve.task              # Task 生命周期 + 状态机 + 关系
gyra_serve.playbook          # Playbook CRUD + 版本 + 策略声明 DSL 校验
gyra_serve.playbook_runtime  # 策略声明执行 + AgentRun 调度（Agent 自主编排，受 gates 约束）
gyra_serve.artifact          # Artifact + 版本 + 检索（含 operation_plan/result/code_project/deliverable_app/notebook）
gyra_serve.delivery          # 四类交付执行（notify/publish/execute/host）
gyra_serve.asset             # Asset CRUD + 版本 + 语义资产特化
gyra_serve.intervention      # 介入请求 + 强制沉淀
gyra_serve.trigger           # TriggerSource 包装调度能力
gyra_serve.action_executor   # execute 类交付的执行器（调 MCP/Skill 执行 operation_plan）
gyra_serve.skill_bundle      # SkillBundle 注册与管理（场景能力载体）
gyra_serve.hosting_runtime   # host 类交付的托管运行时（web_runtime/dashboard_viewer/data_explorer/doc_site/notebook_runtime）
```
...
| `gyra_app/openapi/api_v2/api_v2.py` | 注册新 router（含 workspace / workspace_resource / delivery 等） |
| `gyra_app/initialization/serve_initialization.py` | 注册新 serve 模块 |
...
- Artifact 上传走 `gyra_serve.file` 现有文件服务，返回 `content_ref`
...
| **执行引擎性能（多 Agent 并行）** | 复用现有 `gyra_serve.agent` 并行能力；DSL 的并行块映射到现有并行执行 |
