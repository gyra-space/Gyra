...
AgentResource 配置(type + value)
   ...
   │        GyraSkillResource.__init__        : 查 skill_code/path(I/O)
...
| skill **code/path 解析** | `GyraSkillResource.__init__` 查 Skill service | **旧类构造期** |
...
| Skill(gyra) | skill_code/path 查询 + FS 检查 | ✅ 若 config 已带 code/path | 步骤 4 + 5 |
...
每个阶段前跑 `packages/gyra-core/tests/agent/capabilities/` + `packages/gyra-serve/tests/gyra_serve/agent/capabilities/` + `packages/gyra-core/tests/core/interface/` + react_master 套件。
