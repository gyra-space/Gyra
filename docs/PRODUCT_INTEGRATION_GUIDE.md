...
# 初始化
register_builtin_tools()
config = ConfigManager.init("configs/gyra-proxy-aliyun.toml")
```
...
```python
# packages/gyra-serve/src/gyra_serve/agent/your_agent.py

from gyra_core import PermissionChecker, PRIMARY_PERMISSION
...
```python
from gyra_core import DockerSandbox, SandboxFactory
...
```python
from gyra_core import BatchExecutor, WorkflowBuilder
...
```python
# packages/gyra-ext/src/gyra_ext/agent/custom_agent.py
...
from gyra_serve.agent import AgentBase  # 现有基类
...
```python
# 添加到 packages/gyra-app/src/gyra_app/openapi/api_v1/

from fastapi import APIRouter
from gyra_core import tool_registry, register_builtin_tools, BatchExecutor
...
```python
# 旧方式
from gyra_app.config import Config

# 新方式 - 直接使用 ConfigManager
from gyra_core import ConfigManager
...
# 新方式 - 统一工具系统
from gyra_core import tool_registry, ReadTool, WriteTool
...
```python
from gyra_core import ToolBase, ToolMetadata, ToolResult, ToolCategory, ToolRisk
