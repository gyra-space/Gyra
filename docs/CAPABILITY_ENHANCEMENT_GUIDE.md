...
本文档介绍 Gyra 新增的核心能力模块，帮助开发者快速理解和使用这些功能。
...
```python
from gyra_core import PermissionRuleset, PermissionRule, PermissionAction
...
```python
from gyra_core import PRIMARY_PERMISSION, READONLY_PERMISSION, EXPLORE_PERMISSION, SANDBOX_PERMISSION
...
```python
from gyra_core import PermissionChecker
...
```python
from gyra_core import DockerSandbox, SandboxConfig
...
```python
from gyra_core import SandboxFactory
...
```python
from gyra_core import LocalSandbox
...
```python
from gyra_core import ReadTool
...
```python
from gyra_core import WriteTool
...
```python
from gyra_core import EditTool
...
```python
from gyra_core import GlobTool, GrepTool
...
```python
from gyra_core import BashTool
...
```python
from gyra_core import WebFetchTool
...
```python
from gyra_core import WebSearchTool
...
```python
from gyra_core import BatchExecutor
...
```python
from gyra_core import TaskExecutor
...
```python
from gyra_core import WorkflowBuilder
...
```python
from gyra_core import ConfigLoader, ConfigManager
...
# 验证配置
from gyra_core import ConfigValidator
...
```python
# Python方式
ConfigLoader.generate_default("gyra.json")

# 或使用CLI
# python -m gyra_core.config init -o gyra.json
```
...
async def main():
    # 1. 加载配置
    config = ConfigManager.init("gyra.json")
...
1. 阅读详细API文档：`packages/gyra-core/src/gyra_core/`
2. 查看测试用例：`tests/`
3. 集成到现有Agent：参考 `packages/gyra-serve/`