...
```python
from gyra.agent.core.memory import UnifiedCompactionPipeline, ContextMetricsCollector
...
# 设置 WebSocket 推送回调
from gyra.vis.realtime import get_realtime_pusher
...
### 后端
- `packages/gyra-core/src/gyra/agent/core/memory/context_metrics.py` - 指标收集器
- `packages/gyra-core/src/gyra/agent/core/memory/compaction_pipeline.py` - Pipeline 集成
