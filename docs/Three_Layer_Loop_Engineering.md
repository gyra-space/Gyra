# 三层 Loop 工程时代：Gyra 的技术思考与实践

> 从 Prompt 到 Loop，AI 产品正从"一次性回答"走向"持续参与"。本文梳理 AI 工程的五个演进时代，聚焦当前的 **Loop 工程时代**，并拆解 Gyra 如何通过 **L1 LLM Loop / L2 Agent Loop / L3 业务场景 Loop** 三层嵌套循环，配合业务数据自主飞轮进化，构建一个"团队原生、越用越强"的 AI 产品。

![Gyra 三层 Loop 与数据飞轮全景：左侧三层 Loop 嵌套塔，右侧六个咬合传动的数据飞轮](img/gyra_flywheel_cover.jpg)
...
![Gyra 全局架构：五层技术架构（横向）× 三层 Loop（右侧纵向嵌套）× 三个飞轮闭环（左侧虚线）](img/gyra_global_architecture.jpg)
...
L1 Loop 的核心是 ReAct 范式——**Thinking（思考）→ Act（行动）→ Verify（验证）** 的循环。Gyra 的 [react_master_agent.py](../packages/gyra-core/src/gyra/agent/expand/react_master_agent/react_master_agent.py) 实现了这个循环，由 [base_agent.py](../packages/gyra-core/src/gyra/agent/core/base_agent.py) 的 `generate_reply` 驱动：
...
让 LLM 循环起来容易，让它在 300 步内不崩、不跑飞、不烧钱，才是 Loop 工程的难点。Gyra 用四个机制守护循环（对应上方 L1 图示的外围四个模块）：
...
300 步循环会产生海量历史消息，远超上下文窗口。Gyra 把历史上 7 套压缩机制统一收敛为 [ContextEngine](../packages/gyra-core/src/gyra/agent/expand/react_master_agent/context_engine/engine.py)，构建一条 pipeline：
...
记忆质量需要评测来闭环。Gyra 的评测模块位于 [evaluation.py](../packages/gyra-serve/src/gyra_serve/agent/evaluation/)，提供评测指标驱动记忆质量优化。**评测驱动**的含义是：记忆的晋升、归档、冻结都基于可度量的召回数据，而不是凭启发式规则。
...
L3 Loop 的产出不只是"chat 里的最后一条消息"（对应上方 L3 飞轮的第四、五个节点）。Gyra 定义了四类交付，覆盖产出物的所有命运：
...
三层 Loop 的最终目标是让业务数据**自主飞轮进化**（封面图右侧的六个咬合齿轮）。Gyra 设计了六个咬合传动的飞轮：
...
个人 Agent 的循环止于"任务完成"，团队原生 AI 的循环止于"场景沉淀"。六个飞轮（语义/剧本/上下文/知识/能力/场景）相互咬合，业务数据越跑飞轮越快，空间越用越厚。这种**复利动能**是个人 Agent 无法复制的护城河——它让 Gyra 不只是一个"更强的 Agent 平台"，而是一个"越用越强的团队 AI 飞轮"。
