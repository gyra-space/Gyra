# Gyra ReActMasterV2 Issues Analysis
...
```sql
skill_code: "open-rca-diagnosis-2-0-gyra-c5b0e208"
...
{
  "type": "skill(gyra)",
  "value": "{\"skill_name\":\"open_rca_diagnosis\",\"skillCode\":\"open-rca-diagnosis-2-0-gyra-c5b0e208\"}"
}
```
...
#### Backend Data Flow
1. **vis_window3 Converter** (`gyra_vis_window3_converter.py`)
...
sqlite3 pilot/meta_data/gyra.db "SELECT skill_code, name FROM server_app_skill WHERE name='open_rca_diagnosis';"

# 2. Check app config
cat packages/gyra-serve/src/gyra_serve/building/app/service/gyra_app_define/rca_openrca_app.json | grep -A5 "skill_name"
