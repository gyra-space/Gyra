...
## 三、与 gyra VIS 协议的对比分析

### 3.1 架构差异对比

| 维度 | OpenCode | gyra VIS |
|------|----------|------------|
...
**关键差异**:
- **自动化程度**: OpenCode 自动分解 Part，gyra 手动创建 Block
- **状态同步**: OpenCode 响应式自动更新，gyra 手动推送
- **类型安全**: OpenCode 强类型 Part，gyra 弱类型 VIS 文本
...
**关键差异**:
- **交互性**: OpenCode 支持丰富的终端交互，gyra 依赖前端实现
- **渲染引擎**: OpenCode 自定义终端渲染，gyra 依赖 Web 技术
- **状态反馈**: OpenCode 实时状态更新，gyra 手动状态管理
...
**关键差异**:
- **类型安全**: OpenCode 强类型 Part，gyra 弱类型 Block
- **渲染耦合**: OpenCode 组件化渲染，gyra 前后端分离
- **扩展复杂度**: OpenCode 端到端扩展，gyra 需要前后端协调
