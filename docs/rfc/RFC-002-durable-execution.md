...
class FileStateStore(StateStore):
    """文件后端，按 session_id 落盘到 ${GYRA_HOME}/recovery/{session_id}.json
...
| **S6** | CLI 工具 `gyra recover-session <session_id>`：手动恢复 | ~50 行 | 低 |
...
| AC-2 | `FileStateStore` 默认落盘到 `${GYRA_HOME}/recovery/`，重启后可见 | 集成测试 |
