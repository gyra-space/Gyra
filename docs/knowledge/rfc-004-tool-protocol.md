...
Gyra 现有知识接入方式（`gyra-serve/agent/resource/knowledge.py:92` `KnowledgeSpaceRetrieverResource`）把检索逻辑塞进 Resource 内部，`_retrieve` 直接调 `KnowledgeSpaceRetriever`。这种设计与 llm-wiki.md spec 第 33 行精神不符——spec 强调 LLM 应该直接操作 wiki，不应该有"检索"中间层。
...
**注册机制**：通过 Gyra `ToolRegistry`（`gyra-core/agent/tools/registry.py:242`）注册为 `ToolSource.SYSTEM`，分类 `ToolCategory.SEARCH` / `ToolCategory.DATABASE`。
...
**注册机制**：复用 Gyra `gyra-serve/mcp/` 框架，stdio + HTTP loopback 双形态。

**参数来源**：tool 直接接 `space_slug` 参数，靠 MCP server 端鉴权控制能访问哪些 space。

### 2.4 Handler 共用

内置 Tool 和 MCP Tool 共用同一份 handler 实现（`gyra-core/knowledge/tools/handlers.py`），只是参数来源和鉴权方式不同。
...
```python
# packages/gyra-core/src/gyra/knowledge/resource.py
...
| 旧 | 新 |
|---|---|
| `gyra-serve/agent/resource/knowledge.py:KnowledgeSpaceRetrieverResource` | `gyra-core/knowledge/resource.py:KnowledgeSpaceResource` |
| `gyra-serve/agent/resource/knowledge_pack.py:KnowledgePackResource` | 删除，多 space 通过挂载多个 `KnowledgeSpaceResource` 实现 |
| `gyra-core/agent/resource/knowledge.py:RetrieverResource` 基类 | 删除（仅保留 `Resource` 基类） |

### 3.3 注册

复用 `gyra_app/component_configs.py` 的注册机制：

```python
# packages/gyra-app/src/gyra_app/component_configs.py
...
```python
# packages/gyra-core/src/gyra/knowledge/tools/handlers.py
...
```python
# packages/gyra-core/src/gyra/knowledge/tools/builtin.py

from gyra.agent.tools import ToolBase, ToolMetadata, ToolCategory, ToolSource
...
```python
# packages/gyra-serve/src/gyra_serve/knowledge/mcp.py

from gyra_serve.mcp import McpServer, McpTool
...
| 形态 | 启动命令 | 适用场景 |
|---|---|---|
| stdio | `gyra mcp knowledge --space <slug>` | Claude Desktop / Claude Code 配置 mcp.json |
| HTTP loopback | `gyra mcp knowledge --http --port 7780` | Cursor / Codex 等 HTTP MCP 客户端 |
...
|---|---|---|
| 内置 Tool（Gyra Agent） | tool 参数 | 校验 `space_slug` 在 Agent 挂载的 `KnowledgeSpaceResource` 列表里 |
| MCP stdio（单 space 模式） | 启动时 `--space` 指定 | 不需要运行时鉴权，进程级隔离 |
| MCP HTTP（多 space 模式） | tool 参数 | MCP server 端 token 鉴权 + space ACL |
| HTTP API（Web UI 用） | URL path `/spaces/{id}` | Gyra 现有 JWT 鉴权 |

## 8. 与 Gyra ToolBase 集成

`ToolBase`（`gyra-core/agent/tools/base.py:81`）已提供完整基类，新 tool 直接继承：
...
注册到全局 `tool_registry`（`registry.py:242`），自动暴露给 Gyra Agent 系统。
...
| 文件 | 内容 | 替换为 |
|---|---|---|
| `gyra-serve/agent/resource/knowledge.py` | `KnowledgeSpaceRetrieverResource` | `KnowledgeSpaceResource` |
| `gyra-serve/agent/resource/knowledge_pack.py` | `KnowledgePackResource` | 多个 `KnowledgeSpaceResource` |
| `gyra-core/agent/resource/knowledge.py` | `RetrieverResource` / `RetrieverResourceParameters` | `Resource` 基类直接用 |
| `gyra-serve/rag/retriever/knowledge_space.py` | `KnowledgeSpaceRetriever` | 删除（检索逻辑移到 tool handler） |
| `gyra-serve/rag/service/service.py` 的 `retrieve` / `knowledge_search` 方法 | 旧检索服务 | tool handler 内部调 VaultFS |
...
- [ ] 删除旧 `RetrieverResource` / `KnowledgeSpaceRetrieverResource` / `KnowledgePackResource` 后 Gyra 启动正常
