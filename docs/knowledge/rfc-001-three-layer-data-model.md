...
Gyra 现有 RAG 模块（`gyra-core/rag/` + `gyra-ext/rag/` + `gyra-serve/rag/`）存在以下问题：
...
| `legacy_chunk` | 旧 RAG 迁移 | 一次性 `ks-migrate-from-gyra` 脚本 |
...
一次性脚本 `ks-migrate-from-gyra`：

```
1. 读 gyra.db 的 knowledge_document + document_chunk + knowledge_space_graph_relation
...
**完全替换 mempalace**：
- 删除 `gyra-core/storage/memory/` 整个目录
- 删除 `gyra-ext/storage/memory/mempalace_store.py` 等
- 删除 `pyproject.toml` 的 `mempalace>=3.3.0` 依赖
- agent 对话级记忆改走 L0 verbatim（`extract_mode="convo"`）

### 10.3 与 Gyra Resource 系统的关系

保留 `ResourceType.Knowledge` 枚举（`gyra-core/agent/resource/base.py:40`），但替换实现：
- 删除 `gyra-serve/agent/resource/knowledge.py`（`KnowledgeSpaceRetrieverResource`）
- 删除 `gyra-serve/agent/resource/knowledge_pack.py`（`KnowledgePackResource`）
