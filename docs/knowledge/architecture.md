...
| 旧 Gyra RAG | `ChunkStrategy` 枚举硬编码 | schema.md `## Page Types` 驱动 type 路由 |
| 旧 Gyra RAG | `EmbeddingFactory` 无 model identity 校验 | `embedder_identity` 表 + 三态状态机 + `force_swap` |

---

## 1. 概览

Gyra 新知识系统（模块名 `gyra_serve.knowledge` + `gyra_ext.knowledge`）替换了原有四套分叉存储抽象（`gyra-core/rag` + `gyra-ext/rag` + `gyra-serve/rag` + `mempalace`），统一到一套 VaultFS 接口下，本地模式与分布式模式功能对等。
...
```
gyra-core/src/gyra/knowledge/         协议层（types / schema 解析 / frontmatter / SQLite schema）
gyra-ext/src/gyra_ext/knowledge/      扩展层（VaultFS 实现 + Tool + Extractor + Embedder factory）
gyra-serve/src/gyra_serve/knowledge/  服务层（Service + API + Ingest + Config）
...
1. **HTTP / Tool 层**：参数解析、鉴权、序列化。HTTP 用 FastAPI，Tool 用 Gyra `ToolBase`。
...
```
VaultFS (Protocol, runtime_checkable)        ← gyra-core/knowledge/vaultfs.py
  ▲
  │ implements
  │
BaseVaultFS (ABC, base.py)                  ← gyra-ext/knowledge/vaultfs/base.py
  ▲
  │ extends
  ├┴───────────────┐
...
LLM 调用走 `AIWrapper` + `ModelConfigCache`（Gyra Agent 系统标准路径），支持多模态（图/音）。
...
**Space 注册表**：
- Local：扫描 `~/.gyra/spaces/<slug>/` 目录（启发式：含 `schema.md` 或 `.ks/` 即视为 space）
...
|---|---|---|
| 内置 Tool（Gyra Agent） | tool 参数 | 校验 slug 在 Agent 挂载的 `KnowledgeSpaceResource` 列表 |
| HTTP API（Web UI） | URL path `/spaces/{slug}` | Gyra JWT |
...
```toml
[gyra.serve.knowledge]
local_root = "~/.gyra/spaces"
...
uploads_bucket = "gyra-knowledge"

[gyra.serve.knowledge.distributed]
enabled = true
...
vector_dsn = "postgresql+asyncpg://user:pwd@pg-host:5432/gyra_vec"
```
...
lance_s3_uri = "s3://gyra-vectors/knowledge"
```
...
`packages/gyra-ext/tests/knowledge/vaultfs/conformance.py` 定义约 30 个测试，LocalVaultFS 和 DistributedVaultFS **必须全部通过**。覆盖：
...
KNOWLEDGE_S3_BUCKET=gyra-test \
KNOWLEDGE_EMBEDDER_MODEL=text-embedding-3-small \
pytest packages/gyra-ext/tests/knowledge/vaultfs/test_vector_lifecycle.py
```
...
```
pytest packages/gyra-ext/tests/knowledge/ -q
...
**协议层（gyra-core）**：
- `src/gyra/knowledge/types.py` — Verbat / Document / Edge / DocHit / EmbedderIdentity / ReindexReport 等数据类
- `src/gyra/knowledge/schema.py` — schema.md 解析与校验
- `src/gyra/knowledge/schema_sql.py` — SQLite DDL + chunk_hash migration
- `src/gyra/knowledge/frontmatter.py` — 容错 frontmatter / wikilink / footnote 解析
- `src/gyra/knowledge/vaultfs.py` — VaultFS Protocol

**扩展层（gyra-ext）**：
- `src/gyra_ext/knowledge/vaultfs/base.py` — BaseVaultFS（共享编排，~1200 行）
- `src/gyra_ext/knowledge/vaultfs/local.py` — LocalVaultFS
- `src/gyra_ext/knowledge/vaultfs/distributed.py` — DistributedVaultFS
- `src/gyra_ext/knowledge/vaultfs/_util.py` — chunk_text + path 工具
- `src/gyra_ext/knowledge/vaultfs/vector_store.py` — VectorStore Protocol
- `src/gyra_ext/knowledge/vaultfs/pg_vector_store.py` — pgvector adapter
- `src/gyra_ext/knowledge/vaultfs/vector_milvus.py` — Milvus adapter
- `src/gyra_ext/knowledge/vaultfs/vector_chroma.py` — Chroma adapter
- `src/gyra_ext/knowledge/vaultfs/vector_lancedb.py` — Local LanceDB adapter
- `src/gyra_ext/knowledge/vaultfs/vector_lance_s3.py` — Lance on S3 adapter
- `src/gyra_ext/knowledge/vaultfs/relational/sqlalchemy_store.py` — Postgres/MySQL relational store
- `src/gyra_ext/knowledge/vaultfs/files/s3_store.py` — S3 blob store
- `src/gyra_ext/knowledge/vaultfs/lock/sql_lock.py` — SQL advisory lock
- `src/gyra_ext/knowledge/extractors/__init__.py` — Extractor Protocol + Registry
- `src/gyra_ext/knowledge/extractors/builtin.py` — text/pdf/docx/pptx/image/audio
- `src/gyra_ext/knowledge/embedder_factory.py` — EmbedderCache + get_embedder
- `src/gyra_ext/knowledge/tools/{l0,l1,l2,admin,space,base}.py` — Agent tools

**服务层（gyra-serve）**：
- `src/gyra_serve/knowledge/config.py` — ServeConfig + DistributedConfig
- `src/gyra_serve/knowledge/service/service.py` — Service（registry + _make_vault + embedder hint）
- `src/gyra_serve/knowledge/api/endpoints.py` — FastAPI router
- `src/gyra_serve/knowledge/api/schemas.py` — Pydantic schemas
- `src/gyra_serve/knowledge/ingest.py` — IngestOrchestrator
...
- `packages/gyra-ext/tests/knowledge/vaultfs/test_local.py` — LocalVaultFS 单元
- `packages/gyra-ext/tests/knowledge/vaultfs/test_conformance.py` — 两 backend 共享 conformance
- `packages/gyra-ext/tests/knowledge/vaultfs/test_distributed_conformance.py` — Distributed 集成（skip without env）
- `packages/gyra-ext/tests/knowledge/vaultfs/test_vector_lifecycle.py` — 向量全生命周期（skip without env）
...
- **从旧 gyra.db 迁移脚本**：Task #3 仍 pending
...
```bash
# 1. Local 单元测试（不需外部依赖）
pytest packages/gyra-ext/tests/knowledge/vaultfs/test_local.py -v

# 2. Conformance（LocalVaultFS + 内存仓库）
pytest packages/gyra-ext/tests/knowledge/vaultfs/test_conformance.py -v

# 3. 前端类型检查
cd web && npx tsc --noEmit

# 4. 启动 backend（配置好 ~/.gyra.toml）
python packages/gyra-app/src/gyra_app/dbgpts_cli.py webserver start
...
# 等 done 后查询 pgvector
psql -d gyra -c "SELECT id, document_id FROM ks_vectors_<space_id> LIMIT 10;"
