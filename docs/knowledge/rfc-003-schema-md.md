...
Gyra 现有 RAG 把 chunk 策略硬编码在 `ChunkStrategy` 枚举（`gyra-core/rag/knowledge/base.py:215`），用户改不了。llm_wiki 的 `wiki-schema.ts:27-92` 实现了 schema.md 驱动 page type 路由但没做 relation type。llmwiki 的关系类型硬编码 `cites` / `links_to` 两种。
