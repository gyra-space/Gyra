...
- `WorkspaceResourceEntity`（`gyra_serve/workspace/models/models.py:94`）= 绑定记录，`physical_ref` 指向全局资源本体（connect_config.id / knowledge_space_id / mcp_server_id …）。
- 物化分派（`gyra_serve/workspace/materializer.py:113-122`）：`mcp`/`data_source`/`skill`/`knowledge_space`/`app` → 全局模块取本体 → `AgentResource`。
...
- `DBType` 枚举（`gyra_ext/datasource/schema.py:17-36`）**没有 excel/csv 类型**；已有 sqlite/duckdb 等 `db_path` 文件库先例。
- Excel/CSV 目前只有 ad-hoc 消费：`gyra_ext/agent/agents/open_ta/tools/xls_analysis.py` 载入**临时 DuckDB**（`_chat_excel_tmp/`）用完即弃。
...
**单向依赖（引用完整性，消除"空间 vs 剧本双轨管理"的误解）**：

> **空间 = 注册/治理池，剧本 = 选配/编排子集。剧本能引用的资产/能力必须是空间已绑定的（`workspace_resource`），不允许绕过空间治理直接挂全局。**

- 写路径：`create_playbook` / `update_playbook` 做引用完整性校验——引用命中空间池 → 通过；未命中但全局可解析 → warning（不阻断，兼容存量/seed，提示绑定以获得治理与权限投影）；全局确认不存在（skill/mcp 可确定）→ 报错阻断，防悬空引用。
- 读路径：物化剧本引用时传入 `workspace_id`，引用优先按空间绑定记录物化（type/physical_ref/config 以绑定为准），未绑定引用全局兜底 + 记日志。这使剧本引用与空间绑定物化结果一致，是"剧本权限 = 引用集合投影"的地基。
- 前端：剧本编辑器的技能/资源选择器都只从空间资源池选（同口径），已选但未绑定的历史引用标记「未绑定」并引导到「能力」页绑定。
...
新增 `DBType.excel` / `DBType.csv`（`gyra_ext/datasource/schema.py`），标记为文件库（`is_file_db`），**connector 内部委托 DuckDB**。
...
### 5.2 `DBType` 枚举扩展（`gyra_ext/datasource/schema.py`）
...
connector 层：excel/csv 类型委托 duckdb connector（`db_path` 连接），改动集中在 gyra-ext connector 工厂。
