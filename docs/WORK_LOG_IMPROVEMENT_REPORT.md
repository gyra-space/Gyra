...
```python
from gyra.agent.core.memory.compaction_pipeline import UnifiedCompactionConfig
from gyra.agent.expand.react_master_agent.work_log import create_work_log_manager
...
1. **`packages/gyra-core/src/gyra/agent/core/memory/compaction_pipeline.py`**
   - 创建 `UnifiedCompactionConfig` 类
   - 调整 Layer 2 配置参数
   - 添加向后兼容别名

2. **`packages/gyra-core/src/gyra/agent/expand/react_master_agent/work_log.py`**
   - 使用统一配置
   - 添加内容保护机制
   - 添加自适应触发
   - 添加监控指标收集
...
```bash
# 运行所有测试
pytest packages/gyra-core/tests/agent/ -v

# 运行特定测试
pytest packages/gyra-core/tests/agent/test_work_log.py -v
```
