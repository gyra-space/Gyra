...
-- Version: 0.6.0
-- Generated: 2026-09-06T23:15:05.889091
-- ============================================================
...
CREATE TABLE IF NOT EXISTS `server_app_playbook` (
  ...
  `target_app_code` VARCHAR(128) NULL COMMENT '合约目标专家（gpts_app.app_code）',
  ...
  KEY `ix_server_app_playbook_target_app_code` (`target_app_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `server_app_workspace_conv_link` (
  ...
  `gmt_modified` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `ix_server_app_workspace_conv_link_is_favorited` (`is_favorited`),
...
CREATE TABLE IF NOT EXISTS `server_app_trigger_source` (
  ...
  `config_json` TEXT NULL,
  `target_playbook_id` INT NULL,
  `target_app_code` VARCHAR(128) NULL COMMENT '触发目标专家（gpts_app.app_code）',
  ...
  `gmt_modified` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `ix_server_app_trigger_source_target_app_code` (`target_app_code`),
...
-- Table: gyra_memory_recall_stats
CREATE TABLE IF NOT EXISTS `gyra_memory_recall_stats` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `memory_id` VARCHAR(255) NOT NULL,
  `space_id` VARCHAR(255) NULL,
  `recall_count` INT NOT NULL DEFAULT 0,
  `total_score` FLOAT NOT NULL DEFAULT '0.0',
  `query_hashes` TEXT NULL,
  `recall_days` TEXT NULL,
  `last_recalled` DATETIME NULL,
  `gmt_create` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `ix_gyra_memory_recall_stats_space_id` (`space_id`),
  UNIQUE KEY `ix_gyra_memory_recall_stats_memory_id` (`memory_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Table: server_app_workspace_expert
CREATE TABLE IF NOT EXISTS `server_app_workspace_expert` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `workspace_id` INT NOT NULL,
  `app_code` VARCHAR(128) NOT NULL COMMENT '专家身份（gpts_app.app_code）',
  `role_hint` VARCHAR(256) NULL COMMENT '空间内职责说明（prompt 补丁）',
  `icon` VARCHAR(512) NULL COMMENT '空间级头像覆盖（空则回落 GptsApp.icon）',
  `default_contract_id` INT NULL COMMENT '默认交付合约 id',
  `is_active` TINYINT(1) NOT NULL DEFAULT 1,
  `gmt_create` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_workspace_expert` (`workspace_id`, `app_code`),
  KEY `ix_server_app_workspace_expert_workspace_id` (`workspace_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Table: server_app_workspace_expert_equipment
CREATE TABLE IF NOT EXISTS `server_app_workspace_expert_equipment` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `expert_id` INT NOT NULL COMMENT 'workspace_expert.id',
  `resource_type` VARCHAR(32) NOT NULL COMMENT 'data_source/knowledge_space/mcp/skill（与 workspace_resource.type 对齐）',
  `resource_ref` VARCHAR(255) NOT NULL COMMENT '引用目标（空间资源 name/physical_ref）',
  `config_json` TEXT NULL COMMENT '外挂级参数（如知识库 top_k）',
  `is_active` TINYINT(1) NOT NULL DEFAULT 1,
  `gmt_create` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modified` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_workspace_expert_equipment` (`expert_id`, `resource_type`, `resource_ref`),
  KEY `ix_server_app_workspace_expert_equipment_expert_id` (`expert_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `gyra_serve_llm_usage` (
  ...
  `cached_tokens` INT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `gpts_work_log` (
  ...
  `message_id` VARCHAR(128) NULL COMMENT '关联的 GptsMessage ID (用于重建 action_report)',
  `tool_call_id` VARCHAR(128) NULL COMMENT '工具调用 ID (用于关联 tool message)',
...
CREATE TABLE IF NOT EXISTS `gpts_file_metadata` (
  ...
  `local_path` VARCHAR(1024) NOT NULL DEFAULT '' COMMENT 'The local path of the file',
...
CREATE TABLE IF NOT EXISTS `gpts_app` (
  ...
  `owner_workspace_id` INT NULL COMMENT 'owner workspace id; NULL = global agent/expert',
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_gpts_app` (`app_name`),
  KEY `idx_gpts_app_user_code` (`user_code`),
  KEY `idx_gpts_app_owner_workspace` (`owner_workspace_id`),
  KEY `idx_gpts_app_published` (`published`),
  KEY `ix_gpts_app_owner_workspace_id` (`owner_workspace_id`),
...
CREATE TABLE IF NOT EXISTS `server_app_task` (
  ...
  `playbook_version_id` INT NULL,
  `expert_app_code` VARCHAR(128) NULL COMMENT '执行专家（gpts_app.app_code）',
  `contract_id` INT NULL COMMENT '交付合约 id（playbook 表收窄语义）',
  ...
  KEY `ix_server_app_task_contract_id` (`contract_id`),
  ...
  KEY `ix_server_app_task_expert_app_code` (`expert_app_code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `server_app_artifact` (
  ...
  KEY `ix_server_app_artifact_conv_id` (`conv_id`),
...
-- Table: gyra_serve_ecp_miss_learn
CREATE TABLE IF NOT EXISTS `gyra_serve_ecp_miss_learn` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `workspace_id` VARCHAR(128) NOT NULL DEFAULT 'default',
  `kind` VARCHAR(32) NOT NULL,
  ...
  `trigger` VARCHAR(32) NOT NULL DEFAULT 'agent',
  `learned_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modify` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_ecp_miss_learn_ws` (`workspace_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Table: gyra_serve_ecp_semantic_alignment
CREATE TABLE IF NOT EXISTS `gyra_serve_ecp_semantic_alignment` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `workspace_id` VARCHAR(128) NOT NULL DEFAULT 'default',
  ...
  `gmt_modify` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ecp_alignment_key` (`workspace_id`, `slug`, `entity_name`, `object_id`),
  KEY `idx_ecp_alignment_ws_status` (`workspace_id`, `status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `role` (
  ...
  `scope_type` VARCHAR(16) NOT NULL DEFAULT 'global' COMMENT '角色域: global=全局角色 / space=空间角色(须绑定具体空间)',
  `gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modify` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name` (`name`)
...
CREATE TABLE IF NOT EXISTS `user_role` (
  ...
  `role_id` INT NOT NULL COMMENT 'role.id',
  `scope_id` INT NULL COMMENT '空间级绑定的 workspace.id；NULL=全局绑定',
  `gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `permission_definition` (
  ...
  `scope_type` VARCHAR(16) NOT NULL DEFAULT 'global' COMMENT '权限域: global / space',
  `grantable` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否允许开资源实例级授权',
  `gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `gmt_modify` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name` (`name`)
...
CREATE TABLE IF NOT EXISTS `resource_grant` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL COMMENT 'user.id',
  `permission_key` VARCHAR(128) NOT NULL COMMENT '协议权限 key，如 agent.chat',
  `resource_type` VARCHAR(64) NOT NULL COMMENT '资源类型',
  `resource_id` VARCHAR(255) NOT NULL COMMENT '具体资源实例ID，*表示该类型全部',
  `expires_at` DATETIME NULL COMMENT '过期时间，NULL=永久',
  `granted_by` INT NULL COMMENT '授权人 user.id',
  `gmt_create` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_resource_grant` (`user_id`, `permission_key`, `resource_type`, `resource_id`),
  KEY `ix_resource_grant_user_id` (`user_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
