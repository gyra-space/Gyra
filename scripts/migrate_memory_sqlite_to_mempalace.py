...
for _pkg in ("gyra-core", "gyra-ext", "gyra-serve", "gyra-app"):
...
def _build_embedding_fn(app_config):
    ...
    from gyra.rag.embedding.embeddings import OpenAPIEmbeddings  # type: ignore
...
def _bootstrap(config_file: str):
    ...
    """
    from gyra_app.app import scan_configs, load_config

    scan_configs()

    # Importing the config triggers MemoryStoreConfig subclass registration.
    from gyra_ext.storage.memory.mempalace_store import (  # noqa: F401
...
def _create_store(memory_cfg, embedding_fn, index_name: str):
    ...
    """
    from gyra_ext.storage.memory.mempalace_store import (
        MemPalaceMemoryConfig,
...
def migrate_space(memory_cfg, embedding_fn, index_name: str, db_path: str) -> dict:
    ...
    if not getattr(store, "_use_gyra_embedding", False):
        raise SystemExit(
...
def main():
    ...
    parser.add_argument(
        "--config",
        default="configs/gyra-proxy-openai.toml",
