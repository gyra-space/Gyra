...
# One command to download, install, and start Gyra
#
# Usage:
#   # Quick experience (download and run)
#   curl -fsSL https://raw.githubusercontent.com/yhjun1026/Gyra/main/scripts/quick_start.sh | bash
#
#   # Development mode (run in existing project)
#   ./scripts/quick_start.sh
#
# Environment Variables:
#   GYRA_REPO    - Repository URL (default: https://github.com/yhjun1026/Gyra.git)
#   GYRA_DIR     - Installation directory (default: ~/.gyra for download mode)
#   GYRA_PORT    - Server port (default: 7777)
#   GYRA_BRANCH  - Branch to clone (default: main)
...
# Configuration
REPO_URL="${GYRA_REPO:-https://github.com/yhjun1026/Gyra.git}"
DEFAULT_PORT="${GYRA_PORT:-7777}"
DEFAULT_BRANCH="${GYRA_BRANCH:-main}"
INSTALL_DIR="${GYRA_DIR:-$HOME/gyra}"
...
setup_gyra_env() {
    ...
    if [ -n "${GYRA_HOME:-}" ]; then
        ...
        info "Config directory: $GYRA_HOME (GYRA_HOME)"
        export GYRA_HOME
        return 0
    fi
...
clone_repo() {
    if [ -d "$INSTALL_DIR/.git" ]; then
        ...
        log "Cloning Gyra from $REPO_URL..."
        rm -rf "$INSTALL_DIR"
...
start_server() {
    ...
    log "Starting Gyra server on port $port..."
    echo ""
    echo "========================================"
    echo "  Gyra is starting..."
    echo "  Port: $port"
    ...
    uv run gyra quickstart -p "$port"
}

# Print usage
print_usage() {
    cat << EOF
Gyra Quick Start - One command to download, install, and start
...
Usage:
  ...
  # Custom port
  GYRA_PORT=8888 ./scripts/quick_start.sh
...
Environment Variables:
  GYRA_REPO    Repository URL (default: https://github.com/yhjun1026/Gyra.git)
  GYRA_DIR     Installation directory (default: ~/gyra)
  GYRA_PORT    Server port (default: 7777)
  GYRA_BRANCH  Branch to clone (default: main)
  SKIP_FRONTEND      Skip frontend build (default: false)
