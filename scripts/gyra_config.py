...
sys.path.insert(0, str(Path(__file__).parent.parent / "packages" / "gyra-core" / "src"))

from gyra_core.config import ConfigLoader, ConfigManager, ConfigValidator

def main():
    parser = argparse.ArgumentParser(description="Gyra 配置管理")
    subparsers = parser.add_subparsers(dest="command", help="命令")
    
    init_parser = subparsers.add_parser("init", help="生成默认配置文件")
    init_parser.add_argument("-o", "--output", default="gyra.json", help="输出路径")
