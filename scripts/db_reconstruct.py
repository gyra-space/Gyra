#!/usr/bin/env python3
"""db_reconstruct.py - 从 .recover 的 lost_and_found 重建 gyra.db

背景:gyra.db 的 schema 页(sqlite_master)损坏,.recover 把全部数据塞进
...
nfield 语义(实测):gyra 表均为 id=INTEGER PRIMARY KEY(rowid 别名),
  - nfield = 声明列数(含 id)
...
def setup_paths():
    ...
    for pkg in ("gyra-app", "gyra-core", "gyra-serve", "gyra-ext", "gyra-client"):
...
def create_fresh_schema(fresh_path):
    ...
    for ext in ("", "-wal", "-shm"):
        ...
        if os.path.exists(p):
            os.remove(p)
    from gyra.storage.metadata.db_manager import initialize_db, db
...
def cmd_load(args):
    ...
    if not args.dry_run:
        ...
        print(f"\nintegrity_check: {ic}")
    con.close()
    print("\n若行数正常,把 out 替换为 gyra.db(记得先备份)即可。")


def main():
    ap = argparse.ArgumentParser(description="从 lost_and_found 重建 gyra.db")
