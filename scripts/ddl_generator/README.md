...
DDL Generator 是 Gyra 项目的数据库 schema 管理工具，能够从 SQLAlchemy ORM 模型自动生成多种数据库的 DDL 脚本。
...
# assets/schema/postgresql/gyra.sql
```
...
# 执行生成的 DDL
mysql -h 127.0.0.1 -u root -ptest < assets/schema/mysql/gyra.sql

# 验证表是否创建成功
mysql -h 127.0.0.1 -u root -ptest -e "SHOW TABLES FROM gyra;"
```
...
# 执行生成的 DDL
psql -h 127.0.0.1 -U postgres -f assets/schema/postgresql/gyra.sql
