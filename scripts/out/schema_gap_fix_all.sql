-- Gyra 全量补列修复 SQL（幂等：已存在列会报 duplicate column 被跳过）
-- 来源: upgrade_0.3.0_20260822_to_0.3.0_20260822_0009.sql（全量累积快照）
-- 用法: mysql --force -u<user> -p<pass> <db> < schema_gap_fix_all.sql
-- 注意: --force 让重复列报错后继续，等价于迁移器 tolerate_duplicate 行为
