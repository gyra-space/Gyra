...
# Release Preparation Script for Gyra Project
...
if [ -f "$SCRIPT_DIR/generate_ddl.py" ]; then
    ...
    if $PYTHON_CMD "$SCRIPT_DIR/generate_ddl.py"; then
        ...
        print_info "MySQL Full DDL: assets/schema/mysql/gyra.sql"
        print_info "MySQL Incremental DDL: assets/schema/mysql/upgrades/"
        print_info "PostgreSQL Full DDL: assets/schema/postgresql/gyra.sql"
...
print_info "1. Frontend static files: packages/gyra-app/src/gyra_app/static/web/"
print_info "2. MySQL DDL: assets/schema/mysql/gyra.sql"
print_info "3. PostgreSQL DDL: assets/schema/postgresql/gyra.sql"
