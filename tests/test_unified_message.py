...
class TestUnifiedMessage:
    ...
    def test_from_base_message_ai(self):
        """测试从AIMessage转换"""
        from gyra.core.interface.message import AIMessage
...
class TestUnifiedMessageDAO:
    ...
    def mock_gpts_messages_dao(self):
        ...
        with patch('gyra.storage.unified_message_dao.GptsMessagesDao') as mock:
            yield mock
    
    @pytest.fixture
    def mock_gpts_conversations_dao(self):
        """Mock GptsConversationsDao"""
        with patch('gyra.storage.unified_message_dao.GptsConversationsDao') as mock:
            yield mock
    ...
    async def test_get_messages_by_conv_id(self):
        """测试获取消息"""
        with patch('gyra.storage.unified_message_dao.GptsMessagesDao') as mock_dao:
            mock_entity = Mock()
...
class TestUnifiedStorageAdapter:
    ...
    async def test_storage_conv_adapter_save(self):
        ...
        from gyra.storage.unified_storage_adapter import StorageConversationUnifiedAdapter
        from gyra.core.interface.message import HumanMessage, AIMessage
        ...
        with patch('gyra.storage.unified_storage_adapter.UnifiedMessageDAO') as mock_dao:
...
class TestUnifiedAPI:
    ...
    async def test_get_conversation_messages_api(self):
        ...
        from gyra_serve.unified_api.endpoints import router
        ...
        with patch('gyra_serve.unified_api.endpoints.get_unified_dao') as mock_get_dao:
            mock_dao = AsyncMock()
