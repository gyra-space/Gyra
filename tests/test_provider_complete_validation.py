...
async def run_test_suite():
    ...
    try:
        ...
        with patch("gyra.agent.util.llm.llm_client.OpenAIProvider", TestProvider):
    ...
    try:
        ...
        with patch("gyra.agent.util.llm.llm_client.ClaudeProvider", TestProvider):
            wrapper = AIWrapper(llm_config=config)
            assert wrapper._provider is not None
    ...
    try:
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.OpenAIProvider", TestProvider
        ) as mock:
            wrapper = AIWrapper(llm_config=config)
    ...
    try:
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.OpenAIProvider", return_value=provider
        ):
            wrapper = AIWrapper(llm_config=config)
