...
gyra_serve_path = Path(__file__).parent / "packages" / "gyra-serve" / "src"
if str(gyra_core_path) not in sys.path:
    sys.path.insert(0, str(gyra_core_path))
if str(gyra_serve_path) not in sys.path:
    sys.path.insert(0, str(gyra_serve_path))
...
from gyra.core.interface.llm import ModelRequest, ModelOutput, ModelMetadata
from gyra.agent import AgentContext, ConversableAgent
...
class TestAgentProviderIntegration:
    ...
    async def test_aiwrapper_uses_provider_correctly(self, mock_llm_config):
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.OpenAIProvider",
    ...
    async def test_aiwrapper_handles_empty_response(self, mock_llm_config):
        ...
        with patch("gyra.agent.util.llm.llm_client.OpenAIProvider", EmptyProvider):
            wrapper = AIWrapper(llm_config=mock_llm_config)
