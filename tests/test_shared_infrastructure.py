...
class TestContextArchiver:
    ...
    async def test_process_tool_output_small(self, mock_file_system):
        """测试小输出不归档"""
        from gyra.agent.shared.context_archiver import ContextArchiver
    ...
    async def test_process_tool_output_large(self, mock_file_system):
        """测试大输出自动归档"""
        from gyra.agent.shared.context_archiver import ContextArchiver, ContentType
...
class TestTaskBoardManager:
    ...
    def task_board(self):
        ...
        from gyra.agent.shared.task_board import TaskBoardManager
    ...
    async def test_create_todo(self, task_board):
        """测试创建 Todo"""
        from gyra.agent.shared.task_board import TaskPriority, TaskStatus
    ...
    async def test_update_todo_status(self, task_board):
        """测试更新 Todo 状态"""
        from gyra.agent.shared.task_board import TaskStatus
...
class TestSharedSessionContext:
    ...
    async def test_context_statistics(self, temp_dir):
        """测试上下文统计"""
        from gyra.agent.shared import SharedSessionContext
...
class TestAdapters:
    ...
    async def test_v1_adapter_create(self, shared_context):
        """测试 V1 适配器创建"""
        from gyra.agent.shared import V1ContextAdapter
    ...
    async def test_v2_adapter_create(self, shared_context):
        """测试 V2 适配器创建"""
        from gyra.agent.shared import V2ContextAdapter
