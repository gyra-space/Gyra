...
async def test_local_sandbox():
    ...
    try:
        from gyra_ext.sandbox.local import LocalSandbox

        # 创建临时工作目录 - 这将映射到沙箱内部
        work_dir = tempfile.mkdtemp(prefix="gyra_sandbox_test_")
        ...
        test_content = "Hello, Gyra Sandbox!"
...
async def test_file_system():
    ...
    try:
        ...
        from gyra.agent.core.memory.gpts import FileType

        # 创建临时目录
        base_dir = tempfile.mkdtemp(prefix="gyra_fs_test_")
...
async def test_skill_resource():
    ...
    try:
        from gyra.agent.resource.agent_skills import (
            AgentSkillResource,
            SkillMeta,
            SkillInfo,
        )
...
async def test_agent_message_handling():
    ...
    try:
        from gyra.agent.core.types import AgentMessage, MessageType
    ...
    try:
        ...
        test_results.add_test(
            ...
            details={
                ...
                "note": "Fixed: Added fallback SandboxConfigParameters when gyra_app not available",
            },
        )
...
async def main():
    ...
    logger.info("3. SandboxManager依赖 - 添加了gyra_app配置回退")
    logger.info("")
