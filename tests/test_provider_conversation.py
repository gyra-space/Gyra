...
# Mock dependencies before importing gyra modules
mock_app_config = MagicMock()
sys.modules["gyra_app.config"] = mock_app_config
mock_app_config.SandboxConfigParameters = MagicMock()

mock_awel = MagicMock()
sys.modules["gyra_ext.agent.agents.awel.awel_runner_agent"] = mock_awel
...
# Add the gyra-core package to path
gyra_core_path = Path(__file__).parent / "packages" / "gyra-core" / "src"
if str(gyra_core_path) not in sys.path:
    sys.path.insert(0, str(gyra_core_path))

from gyra.agent.core.llm_config import AgentLLMConfig
from gyra.agent.util.llm.llm_client import AIWrapper, AgentLLMOut
from gyra.agent.util.llm.provider.base import LLMProvider
from gyra.core.interface.llm import (
    ModelRequest,
    ModelOutput,
    ModelMetadata,
...
class TestProviderInitialization:
    ...
    def test_openai_provider_initialization(self):
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.OpenAIProvider", MockOpenAIProvider
        ):
    ...
    def test_claude_provider_initialization(self):
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.ClaudeProvider", MockClaudeProvider
        ):
            wrapper = AIWrapper(llm_config=config)
...
class TestProviderConversation:
    ...
    async def test_provider_error_handling(self):
        ...
        with patch("gyra.agent.util.llm.llm_client.OpenAIProvider", ErrorProvider):
            wrapper = AIWrapper(llm_config=config)
    ...
    async def test_temperature_and_max_tokens_configuration(self):
        ...
        with patch(
            "gyra.agent.util.llm.llm_client.OpenAIProvider", CapturingProvider
        ):
            wrapper = AIWrapper(llm_config=config)
