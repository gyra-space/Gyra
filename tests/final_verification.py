...
try:
    from gyra.agent.core.profile.base import ProfileConfig
    import inspect
...
try:
    with open("packages/gyra-core/src/gyra/agent/core/agent_manage.py", "r") as f:
        content = f.read()
...
try:
    with open(
        "packages/gyra-serve/src/gyra_serve/agent/agents/chat/agent_chat.py", "r"
    ) as f:
        content = f.read()

    checks = [
        ("导入resolve_agent_name", "from gyra.agent.core.agent_alias import"),
