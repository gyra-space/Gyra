...
const ChatSider: React.FC<{
  ...
  listLoading: boolean;
  order: React.MutableRefObject<number>;
  isCollapsed?: boolean; //对话列表初始化是否收起
}> = ({ dialogueList = [], refresh, historyLoading, listLoading, order, isCollapsed = false }) => {
  const searchParams = useSearchParams();
  const scene = searchParams?.get('scene') ?? '';
  const { t } = useTranslation();
  const { mode } = useContext(ChatContext);
  const [collapsed, setCollapsed] = useState<boolean>(isCollapsed || scene === 'chat_dashboard');
        ...
        <Flex flex={1} vertical={true} className='overflow-y-auto'>
          <MenuItem
            item={{
              label: t('assistant'),
              key: 'default',
              icon: <Image src='/LOGO_SMALL.png' alt='Gyra' width={24} height={24} className='flex-1' />,
              default: true,
            }}
            order={order}
          />
