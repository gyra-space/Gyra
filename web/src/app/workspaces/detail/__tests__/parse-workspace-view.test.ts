...
describe('deliverable_files / task_files(追问轮不丢前轮交付物)', () => {
  ...
  test('同 file_id 内容真正变化(新版本)时保留最新内容与对应 ts', () => {
    ...
    const view = parseWorkspaceView({
      ...baseChunk,
      deliverable_files: [
        { file_id: 'f1', file_name: 'report-v2.pdf', file_size: 2048, mime_type: 'application/pdf', render_type: 'pdf', ts: '2026-08-10T10:00:00' },
      ],
    }, prev);
    expect(view.deliverable_files).toHaveLength(1);
    expect(view.deliverable_files![0].file_name).toBe('report-v2.pdf');
    expect(view.deliverable_files![0].ts).toBe('2026-08-10T10:00:00');
  });
