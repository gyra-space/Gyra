...
- **开发前先取数**：动手写 code 前，先对每个计划查询用 hook 工具（`app_card_preview` / `db`）执行一次取数，确认字段名与样例值，再据此开发。
- **交付前逐个验证**：每个 `metric` 和 `sql` 都必须用 hook 工具执行一次，确认 `trust !== "none"`、`rows` 有值、字段名与 `code` 中访问一致；任一查询取不到数据或字段不匹配，就修到通过再交付。
...
**交付守门**
- 最后一步**必须运行**随技能分发的 `validate.py`，输出 `[OK]` 且退出码为 0 才能交付；失败就修复后重跑，**不自检不交付**。
